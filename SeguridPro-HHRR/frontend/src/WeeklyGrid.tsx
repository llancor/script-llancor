type Props={
  ...
  onCreate:(guardia:any,date:Date)=>void;onEdit:(shift:any)=>void;canEdit:boolean;
};
...
export default function WeeklyGrid({active,guardias,items,days,config,onCreate,onEdit,canEdit}:Props){
  ...
  return <section className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/80">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div><h2 className="font-bold">Planificación semanal</h2><p className="text-xs text-slate-500">{canEdit?'Selecciona una celda para programar o editar el turno.':'Vista de turnos asignados.'}</p></div>
    ...
    <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
      <table className="w-full min-w-[980px] border-collapse text-xs">
        ...
        <tbody>{guardias.map(g=><tr key={g.id}><th className="sticky left-0 z-10 border-b border-r bg-white p-3 text-left dark:border-slate-700 dark:bg-slate-800"><span className="block truncate font-bold text-slate-900 dark:text-slate-100">{g.nombre}</span><span className="mt-1 block truncate font-normal text-slate-400 dark:text-slate-300">{g.rango||'Guardia'}</span></th>{days.map(d=>{
          ...
          return <td key={key(d)} className="border-b border-r p-1.5 dark:border-slate-700"><button disabled={!canEdit} onClick={()=>canEdit&&onEdit(shift)} style={{backgroundColor:`${color}18`,borderColor:color,color}} className="min-h-16 w-full rounded-lg border-l-4 p-2 text-left transition hover:shadow-sm"><b className="block truncate">{shift.tipo_turno}</b><span className="mt-1 block whitespace-nowrap text-[10px]">{shift.hora_inicio}–{shift.hora_fin}</span>{shifts.length>1&&<span className="mt-1 block text-[9px] font-bold">+{shifts.length-1} turno</span>}</button></td>;
        })}</tr>)}</tbody>
      </table>
    </div>
  </section>;
