...
import { audit, db, encryptSecret, publicUser, sendEmail, sendTelegram } from './lib.js';
...
app.get('/api/dashboard', auth, asyncHandler(async (req, res) => {
    ...
    const all = req.user?.role === 'admin' || req.user?.permisos?.ver_registros === 'todos';
    const scope = (model) => { if (all)
        return {}; const creator = { created_by_id: req.user.id }; const guardiaId = req.user?.guardia_id; if (!guardiaId)
        return creator; if (model === 'guardia')
        return { OR: [creator, { id: guardiaId }] }; return { OR: [creator, { guardia_id: guardiaId }] }; };
...
app.post('/api/guardias/:id/activar-acceso', auth, admin, asyncHandler(async (req, res) => { const guard = await db.guardia.findUniqueOrThrow({ where: { id: req.params.id }, include: { usuario: true } }); if (guard.usuario) {
    if (!guard.usuario.enabled) {
        await db.user.update({ where: { id: guard.usuario.id }, data: { enabled: true } });
        return res.json({ message: 'Acceso web reactivado correctamente' });
    }
    return res.status(409).json({ message: 'Este guardia ya tiene un perfil web activo' });
} const email = String(req.body.email || guard.email || '').trim().toLowerCase(), password = String(req.body.password || ''); if (!/^\S+@\S+\.\S+$/.test(email))
    return res.status(400).json({ message: 'El guardia necesita un correo válido' }); if (password.length < 10)
    return res.status(400).json({ message: 'La contraseña temporal debe tener al menos 10 caracteres' }); if (await db.user.findUnique({ where: { email } }))
    return res.status(409).json({ message: 'Ya existe otro usuario con ese correo' }); const permisos = { guardias: false, turnos: false, relevos: true, rondas: true, recintos: false, entradas: true, reportes: true, alertas: true, rrhh: false, usuarios: false, configuracion: false, editar_entradas: true, editar_reportes: true, editar_alertas: true, eliminar_entradas: false, eliminar_reportes: false, eliminar_alertas: false, ver_registros: 'propios' }; const user = await db.user.create({ data: { full_name: guard.nombre, email, password: await bcrypt.hash(password, 12), role: 'guardia', rango: guard.rango, telefono: guard.telefono, permisos, guardia_id: guard.id, foto_url: guard.foto_url, email_verified: true, enabled: true, must_change_password: true } }); if (req.body.send_invitation)
    sendEmail(email, 'Acceso activado en Seguridad', `Hola ${guard.nombre}. Tu acceso fue activado. Usuario: ${email}\nContraseña temporal: ${password}\nDeberás cambiarla al ingresar.`).catch(console.error); await audit('activar_acceso_guardia', { userId: req.user.id, entity: 'guardia', entityId: guard.id, detail: { usuario: user.id }, ip: req.ip, userAgent: req.get('user-agent') }); res.status(201).json({ message: 'Perfil web creado y activado correctamente', user: publicUser(user) }); }));
...
const safeConfig = (c) => ({ ...c, smtp_password: undefined, telegram_bot_token: undefined, nextcloud_password: undefined, smtp_password_configured: !!c.smtp_password, telegram_token_configured: !!c.telegram_bot_token, nextcloud_password_configured: !!c.nextcloud_password });
...
app.post('/api/turnos/programar', auth, permit('turnos'), asyncHandler(async (req, res) => {
    ...
    if (!guardia_id || !guardia_nombre || !fecha || !hora_inicio || !hora_fin)
        return res.status(400).json({ message: 'Guardia, fecha y horario son obligatorios' });
    if (!['Manana', 'Tarde', 'Dia', 'Noche', 'Personalizado'].includes(tipo_turno))
        return res.status(400).json({ message: 'Tipo de turno inválido' });
    if (hora_inicio === hora_fin)
        return res.status(400).json({ message: 'La hora de inicio y término deben ser diferentes' });
    const lunch = Number(horas_colacion);
    if (!Number.isFinite(lunch) || lunch < 0 || lunch >= 24)
        return res.status(400).json({ message: 'Las horas de colación deben estar entre 0 y menos de 24' });
    if (!['dia', 'semana'].includes(periodo))
        return res.status(400).json({ message: 'El período de programación no es válido' });
    ...
    if (periodo === 'semana') {
        if (!Array.isArray(dias_semana) || !dias_semana.length)
            return res.status(400).json({ message: 'Selecciona al menos un día de trabajo' });
        const selected = [...new Set(dias_semana.map(Number))].filter(day => Number.isInteger(day) && day >= 1 && day <= 7);
        if (!selected.length)
            return res.status(400).json({ message: 'Los días seleccionados no son válidos' });
...
app.post('/api/turnos/copiar-semana', auth, permit('turnos'), asyncHandler(async (req, res) => { const { fecha_origen, fecha_destino } = req.body; if (!fecha_origen || !fecha_destino)
    return res.status(400).json({ message: 'Indica semana de origen y destino' }); const origin = new Date(`${fecha_origen}T00:00:00.000Z`), target = new Date(`${fecha_destino}T00:00:00.000Z`), end = new Date(origin); end.setUTCDate(end.getUTCDate() + 7); const source = await db.turno.findMany({ where: { fecha: { gte: origin, lt: end }, estado: { not: 'Cancelado' } } }); if (!source.length)
    return res.status(404).json({ message: 'La semana de origen no tiene turnos' }); const offset = target.getTime() - origin.getTime(); for (const shift of source)
    if (shift.guardia_id)
...
app.post('/api/turnos/programar-masivo', auth, permit('turnos'), asyncHandler(async (req, res) => { const { guardia_ids, fecha, hora_inicio, hora_fin, tipo_turno = 'Dia', recinto_id } = req.body; if (!Array.isArray(guardia_ids) || !guardia_ids.length)
    return res.status(400).json({ message: 'Selecciona al menos un guardia' }); const guards = await db.guardia.findMany({ where: { id: { in: guardia_ids }, estado: 'Activo' } }), site = recinto_id ? await db.recinto.findUnique({ where: { id: recinto_id } }) : null, day = new Date(`${fecha}T00:00:00.000Z`); for (const guard of guards)
    await validateShifts(guard.id, [{ fecha: day, hora_inicio, hora_fin }]); await db.turno.createMany({ data: guards.map(guard => ({ guardia_id: guard.id, guardia_nombre: guard.nombre, recinto_id: site?.id, recinto_nombre: site?.nombre, tipo_turno, fecha: day, hora_inicio, hora_fin, estado: 'Programado', created_by_id: req.user.id })) }); res.status(201).json({ message: `Se programaron ${guards.length} guardias`, count: guards.length }); }));
...
app.get('/api/turnos/exportar', auth, permit('turnos'), asyncHandler(async (req, res) => { const shifts = await db.turno.findMany({ orderBy: [{ fecha: 'asc' }, { hora_inicio: 'asc' }] }); const rows = [['Fecha', 'Guardia', 'Recinto', 'Jornada', 'Inicio', 'Fin', 'Estado'], ...shifts.map(x => [x.fecha.toISOString().slice(0, 10), x.guardia_nombre, x.recinto_nombre || x.ubicacion || '', x.tipo_turno, x.hora_inicio, x.hora_fin, x.estado])]; if (req.query.formato === 'pdf') {
    const esc = (v) => String(v).replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
    res.type('html').send(`<!doctype html><meta charset="utf-8"><title>Turnos</title><style>body{font:12px Arial}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:6px}th{background:#0f766e;color:white}</style><h1>Planificación de turnos</h1><table>${rows.map((r, i) => `<tr>${r.map(v => `<${i ? 'td' : 'th'}>${esc(v)}</${i ? 'td' : 'th'}>`).join('')}</tr>`).join('')}</table><script>print()</script>`);
    return;
} res.setHeader('Content-Disposition', 'attachment; filename="turnos.csv"'); res.type('text/csv').send('\ufeff' + rows.map(r => r.map(v => `"${String(v).replaceAll('"', '""')}"`).join(';')).join('\n')); }));
...
app.put('/api/profile', auth, asyncHandler(async (req, res) => { const { password, current_password, ...input } = req.body; const allowed = ['full_name', 'telefono', 'cargo', 'foto_url']; const data = Object.fromEntries(Object.entries(input).filter(([key]) => allowed.includes(key))); const me = await db.user.findUniqueOrThrow({ where: { id: req.user.id } }); if (password) {
    ...
    return res.status(400).json({ message: 'Debes definir una nueva contraseña para continuar' }); const updated = await db.$transaction(async (transaction) => { const user = await transaction.user.update({ where: { id: me.id }, data }); if (me.guardia_id)
    await transaction.guardia.update({ where: { id: me.guardia_id }, data: { nombre: data.full_name ?? me.full_name, telefono: data.telefono ?? me.telefono, foto_url: 'foto_url' in data ? data.foto_url : me.foto_url } }); return user; }); res.json(publicUser(updated)); }));
...
app.use((err, req, res, next) => { console.error(err); audit('error_api', { userId: req.user?.id, entity: req.method, entityId: req.originalUrl, detail: { message: err.message }, ip: req.ip, userAgent: req.get?.('user-agent') }); res.status(err.name === 'ZodError' ? 400 : err.status || 500).json({ message: err.issues?.[0]?.message || err.message || 'Error interno' }); });
async function migrateCompatibility() {
    ...
    if (!userColumns.has('guardia_id'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN guardia_id VARCHAR(30) NULL UNIQUE AFTER permisos');
    if (!userColumns.has('foto_url'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN foto_url MEDIUMTEXT NULL AFTER guardia_id');
    if (!userColumns.has('must_change_password'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN must_change_password BOOLEAN NOT NULL DEFAULT FALSE AFTER foto_url');
    if (!userColumns.has('failed_login_attempts'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN failed_login_attempts INT NOT NULL DEFAULT 0 AFTER enabled');
    if (!userColumns.has('locked_until'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN locked_until DATETIME NULL AFTER failed_login_attempts');
    if (!userColumns.has('last_login_at'))
        await db.$executeRawUnsafe('ALTER TABLE users ADD COLUMN last_login_at DATETIME NULL AFTER locked_until');
    ...
    if (!eventColumns.has('archivo_cloud_path'))
        await db.$executeRawUnsafe('ALTER TABLE eventos_rrhh ADD COLUMN archivo_cloud_path VARCHAR(500) NULL AFTER archivo_url');
    if (!eventColumns.has('notificar_email'))
        await db.$executeRawUnsafe('ALTER TABLE eventos_rrhh ADD COLUMN notificar_email BOOLEAN NOT NULL DEFAULT FALSE AFTER confidencial');
    if (!eventColumns.has('notificar_telegram'))
        await db.$executeRawUnsafe('ALTER TABLE eventos_rrhh ADD COLUMN notificar_telegram BOOLEAN NOT NULL DEFAULT FALSE AFTER notificar_email');
    if (!eventColumns.has('inicio_notificado_at'))
        await db.$executeRawUnsafe('ALTER TABLE eventos_rrhh ADD COLUMN inicio_notificado_at DATETIME NULL AFTER notificar_telegram');
    if (!eventColumns.has('termino_notificado_at'))
        await db.$executeRawUnsafe('ALTER TABLE eventos_rrhh ADD COLUMN termino_notificado_at DATETIME NULL AFTER inicio_notificado_at');
    ...
    const guardiaPhoto = await db.$queryRawUnsafe("SELECT DATA_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'guardias' AND COLUMN_NAME = 'foto_url'");
    ...
    const turnoLunchColumns = await db.$queryRawUnsafe("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'turnos'");
    if (!turnoLunchColumns.some(column => column.COLUMN_NAME === 'horas_colacion'))
        await db.$executeRawUnsafe('ALTER TABLE turnos ADD COLUMN horas_colacion DECIMAL(4,2) NOT NULL DEFAULT 0 AFTER hora_fin');
}
async function start() {
    ...
    for (let attempt = 1; attempt <= 15; attempt++) {
        try {
            ...
            lastError = error;
            console.warn(`Base de datos no disponible (intento ${attempt}/15). Reintentando en 3 segundos...`);
    ...
    if (!await db.user.findUnique({ where: { email: adminEmail } })) {
        ...
        if (!initialPassword || initialPassword.length < 12)
            throw new Error('INITIAL_ADMIN_PASSWORD debe existir y tener al menos 12 caracteres');
