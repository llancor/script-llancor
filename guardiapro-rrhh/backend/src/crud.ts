...
const ownedWhere=(req:any,model:string)=>{if(canSeeAll(req))return{};const creator={created_by_id:req.user!.id};const guardiaId=req.user?.guardia_id;if(!guardiaId)return creator;if(model==='guardia')return{OR:[creator,{id:guardiaId}]};if(model==='relevo')return{OR:[creator,{guardia_saliente_id:guardiaId},{guardia_entrante_id:guardiaId}]};if(['turno','ronda','entrada','reporte','alerta'].includes(model))return{OR:[creator,{guardia_id:guardiaId}]};return creator;};
async function ensureVisible(req:any,res:any){
  const item=await (db as any)[req.params.model].findFirst({where:{AND:[{id:req.params.id},ownedWhere(req,req.params.model)]}});
...
crud.post('/:model',asyncHandler(async(req,res)=>{
  ...
  if(req.params.model==='guardia'&&req.body.crear_acceso){
    ...
    const permisos={guardias:false,turnos:false,relevos:true,rondas:true,recintos:false,entradas:true,reportes:true,alertas:true,usuarios:false,configuracion:false,ver_registros:'propios'};
...
crud.put('/:model/:id',asyncHandler(async(req,res)=>{if(!await ensureVisible(req,res))return;const data=normalize(req.params.model,req.body,true);delete data.id;delete data.created_at;delete data.updated_at;delete data.created_by_id;const item=await (db as any)[req.params.model].update({where:{id:req.params.id},data});if(req.params.model==='guardia')await db.user.updateMany({where:{guardia_id:item.id},data:{full_name:item.nombre,telefono:item.telefono,rango:item.rango,foto_url:item.foto_url}});req.app.get('io')?.emit(`${req.params.model}:updated`,item);res.json(item);}));
crud.delete('/:model/:id',asyncHandler(async(req,res)=>{if(!await ensureVisible(req,res))return;await (db as any)[req.params.model].delete({where:{id:req.params.id}});res.status(204).end();}));
