...
crud.post('/:model', asyncHandler(async (req, res) => { const data = normalize(req.params.model, { ...req.body, created_by_id: req.user.id }); const item = await db[req.params.model].create({ data }); req.app.get('io')?.emit(`${req.params.model}:created`, item); res.status(201).json(item); }));
