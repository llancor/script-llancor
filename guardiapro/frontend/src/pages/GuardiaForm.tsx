...
export default function GuardiaForm({fields,initial,onSave,onCancel}:{fields:Field[];initial?:any;onSave:(v:any)=>Promise<void>;onCancel:()=>void}){
  ...
  const save=async(value:any)=>{
    ...
    if(createAccess&&password.length<8)throw new Error('La contraseña temporal debe tener al menos 8 caracteres');
    await onSave({...value,foto_url:photo,crear_acceso:createAccess,password_temporal:password,enviar_invitacion:sendInvitation});
  };
  return <>
    ...
    {!initial&&<section className="mb-5 rounded-xl border border-teal-200 bg-teal-50/60 p-4 dark:border-teal-900 dark:bg-teal-950/30">
      <label className="flex items-center gap-3 font-bold"><input type="checkbox" checked={createAccess} onChange={event=>setCreateAccess(event.target.checked)}/><KeyRound size={18} className="text-teal-600"/>Crear acceso a la plataforma</label>
      {createAccess&&<div className="mt-4 space-y-3"><p className="text-xs text-slate-600 dark:text-slate-300">Se creará una cuenta vinculada con rol Guardia, permisos predeterminados y acceso solamente a sus registros.</p><label className="block text-sm font-semibold">Contraseña temporal<input type="password" minLength={8} required value={password} onChange={event=>setPassword(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white p-2.5 dark:border-slate-700 dark:bg-slate-900"/></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={sendInvitation} onChange={event=>setSendInvitation(event.target.checked)}/>Enviar los datos de acceso por correo</label><p className="text-xs text-amber-700 dark:text-amber-300">El guardia deberá cambiar esta contraseña al iniciar sesión.</p></div>}
    </section>}
