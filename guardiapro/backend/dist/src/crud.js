...
crud.post('/:model', asyncHandler(async (req, res) => { if (req.params.model === 'relevo' && req.body.guardia_saliente_id === req.body.guardia_entrante_id)
    return res.status(400).json({ message: 'El guardia entrante debe ser diferente del guardia saliente' }); const data = normalize(req.params.model, { ...req.body, created_by_id: req.user.id }); const item = await db[req.params.model].create({ data }); req.app.get('io')?.emit(`${req.params.model}:created`, item); notifyCreated(req.params.model, item).catch(error => console.error(`No se pudo notificar ${req.params.model}:`, error)); res.status(201).json(item); }));
crud.put('/:model/:id', asyncHandler(async (req, res) => { const data = normalize(req.params.model, req.body, true); delete data.id; delete data.created_at; delete data.updated_at; delete data.created_by_id; const item = await db[req.params.model].update({ where: { id: req.params.id }, data }); req.app.get('io')?.emit(`${req.params.model}:updated`, item); res.json(item); }));
