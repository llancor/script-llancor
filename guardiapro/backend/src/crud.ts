...
const allowed=['guardia','recinto','turno','ronda','entrada','reporte','alerta'] as const;
const dateFields:Record<string,string[]>= {guardia:['fecha_ingreso'],turno:['fecha'],ronda:['fecha_hora_inicio','fecha_hora_fin'],entrada:['hora_entrada','hora_salida'],reporte:['fecha'],alerta:['fecha']};
...
crud.post('/:model',asyncHandler(async(req,res)=>{const data=normalize(req.params.model,{...req.body,created_by_id:req.user!.id});const item=await (db as any)[req.params.model].create({data});req.app.get('io')?.emit(`${req.params.model}:created`,item);if(req.params.model==='alerta')sendTelegram(`<b>🚨 ${item.titulo}</b>\nNivel: ${item.nivel}\n${item.mensaje||''}\n📍 ${item.ubicacion||'Sin ubicación'}`).catch(error=>console.error('No se pudo notificar por Telegram:',error));res.status(201).json(item);}));
