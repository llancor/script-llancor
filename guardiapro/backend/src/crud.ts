...
async function ensureVisible(req:any,res:any,write=false){
  ...
  if(!item){res.status(write?403:404).json({message:write?'No tienes permiso para modificar este registro':'Registro no encontrado o fuera de tu alcance'});return null}
  return item;
}
