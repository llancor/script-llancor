...
download_project(){
  ...
  if [[ -d "$INSTALL_ROOT/.git" ]]; then
    ...
    printf 'Elimínala, muévela o define otra ruta con SEGURIDPRO_INSTALL_DIR.\n'
    return 1
  else
...
install_app(){
  ...
  printf "\n${B}Construyendo y arrancando SeguridPro...${N}\n"
...
update_app(){
  ...
  if ! project_ready; then
    if [[ -f "$INSTALL_ROOT/$PROJECT_SUBDIR/docker-compose.yml" ]]; then
      ...
      printf "${R}No se encontró una instalación de SeguridPro en $INSTALL_ROOT/$PROJECT_SUBDIR.${N}\n"
      return 1
    fi
  fi
  if ! has git; then
  ...
  if dc up -d --build --remove-orphans; then
    printf "${G}SeguridPro fue actualizado correctamente. La base de datos se conservó.${N}\n"
    dc ps
    show_url
  else
