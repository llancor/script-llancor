...
app.post('/api/users/invite',auth,admin,asyncHandler(async(req,res)=>{const {password,send_invitation,...data}=req.body;if(!password||password.length<8)return res.status(400).json({message:'La contraseña debe tener al menos 8 caracteres'});if(data.role==='guardia')data.permisos={editar_entradas:true,editar_reportes:true,editar_alertas:true,eliminar_entradas:false,eliminar_reportes:false,eliminar_alertas:false,...(data.permisos||{})};const user=await db.user.create({data:{...data,password:await bcrypt.hash(password,12),email_verified:true,enabled:data.enabled??true,must_change_password:true}});if(send_invitation)await sendEmail(user.email,'Cuenta creada en GuardiaPro',`Tu cuenta fue creada. Ingresa con ${user.email} y la contraseña proporcionada por el administrador. Deberás cambiarla al ingresar.`);res.status(201).json(publicUser(user));}));
