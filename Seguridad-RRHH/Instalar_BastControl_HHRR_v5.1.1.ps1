...
$RepoUrl = if ($env:BASTCONTROL_REPO_URL) { $env:BASTCONTROL_REPO_URL } else { 'https://github.com/llancor/Bast-Control-Acceso.git' }
$InstallRoot = if ($env:BASTCONTROL_INSTALL_DIR) { $env:BASTCONTROL_INSTALL_DIR } else { Join-Path $env:USERPROFILE 'bastcontrol' }
$ProjectSubdir = 'BastControl'
...
function Write-Title {
    ...
    Write-Host '  BastControl - Administrador Windows' -ForegroundColor Cyan
    Write-Host '========================================' -ForegroundColor Cyan
    Write-Host
}
...
function Initialize-Env {
    ...
    if(Test-Path $template){Copy-Item $template $file}else{[IO.File]::WriteAllLines($file,@('MYSQL_PASSWORD=','MYSQL_ROOT_PASSWORD=','JWT_SECRET=','APP_URL=http://localhost:8080','HTTP_PORT=8080','GOOGLE_CLIENT_ID=','SMTP_HOST=','SMTP_PORT=587','SMTP_USER=','SMTP_PASS=','MAIL_FROM=BastControl <no-reply@bastcontrol.local>'),[Text.UTF8Encoding]::new($false))}
    Set-EnvValue MYSQL_PASSWORD (New-Secret 24);Set-EnvValue MYSQL_ROOT_PASSWORD (New-Secret 24);Set-EnvValue JWT_SECRET (New-Secret 48);Set-EnvValue INITIAL_ADMIN_EMAIL 'admin@bastcontrol.com';Set-EnvValue INITIAL_ADMIN_PASSWORD (New-Secret 12);Set-EnvValue HTTP_PORT '8080';Set-EnvValue APP_URL 'http://localhost:8080'
}
...
function Get-Project {
    ...
    Write-Host 'Descargando BastControl desde GitHub...' -ForegroundColor Cyan
    $tempRoot = Join-Path $env:TEMP ("bastcontrol-" + [guid]::NewGuid().ToString('N'))
    try {
...
function Install-App {
    ...
    Write-Host 'Construyendo y arrancando BastControl...' -ForegroundColor Cyan;Invoke-Compose up -d --build
    if($LASTEXITCODE -eq 0){Write-Host 'BastControl instalado correctamente.' -ForegroundColor Green;Show-Url}else{Write-Host 'La instalación falló. Revisa los registros.' -ForegroundColor Red;Invoke-Compose logs --tail=100}
}
...
function Show-InitialCredentials {
    ...
    Write-Host
    Initialize-Env
    Write-Host 'Usuario: ' -NoNewline;Write-Host (Get-EnvValue INITIAL_ADMIN_EMAIL 'admin@bastcontrol.com') -ForegroundColor White
...
function Update-App {
    Write-Title
    Write-Host 'Actualizar BastControl' -ForegroundColor Cyan
    Write-Host
    ...
    if(-not (Project-Ready)){
        Write-Host "No se encontró BastControl en $InstallRoot." -ForegroundColor Red
        return
    }

    Write-Host 'Descargando únicamente BastControl desde GitHub...' -ForegroundColor Cyan
    $tempRoot = Join-Path $env:TEMP ("bastcontrol-update-" + [guid]::NewGuid().ToString('N'))
    try {
        git clone --depth 1 --filter=blob:none --sparse --branch main $RepoUrl $tempRoot
        if($LASTEXITCODE -ne 0){Write-Host 'No fue posible descargar la actualización.' -ForegroundColor Red;return}
        git -C $tempRoot sparse-checkout set $ProjectSubdir
        if($LASTEXITCODE -ne 0){Write-Host 'No fue posible seleccionar la carpeta BastControl.' -ForegroundColor Red;return}
    ...
    if($LASTEXITCODE -eq 0){Write-Host 'BastControl actualizado. La base de datos y el archivo .env se conservaron.' -ForegroundColor Green;Invoke-Compose ps;Show-Url}else{Write-Host 'La actualización falló. Revisa los registros.' -ForegroundColor Red}
}

while($true){Write-Title;Write-Host 'INSTALACIÓN' -ForegroundColor Cyan;Write-Option 1 'Instalar dependencias';Write-Option 2 'Instalar BastControl';Write-Host;Write-Host 'ADMINISTRACIÓN' -ForegroundColor Cyan;Write-Option 3 'Estado y control del servicio';Write-Option 4 'Cambiar puerto de Docker';Write-Option 5 'Ver URL y puerto';Write-Option 6 'Gestión de usuarios';Write-Host;Write-Host 'MANTENIMIENTO' -ForegroundColor Cyan;Write-Option 7 'Desinstalar';Write-Option 8 'Reparar base de datos';Write-Option 9 'Ver credenciales iniciales';Write-Option 10 'Actualizar BastControl';Write-Option 0 'Salir';switch(Read-Host 'Selecciona una opción'){'1'{Install-Dependencies;Pause-Menu}'2'{Install-App;Pause-Menu}'3'{Service-Menu}'4'{Change-Port;Pause-Menu}'5'{Write-Title;Show-Url;Pause-Menu}'6'{User-Menu}'7'{Uninstall-App;Pause-Menu}'8'{Repair-Database;Pause-Menu}'9'{Show-InitialCredentials;Pause-Menu}'10'{Update-App;Pause-Menu}'0'{exit}}}
