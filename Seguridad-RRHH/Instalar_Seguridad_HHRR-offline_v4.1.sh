...
download_project(){
  ...
  title
  printf "${B}Descargando Seguridad RRHH desde GitHub...${N}\n\n"
  ...
  temp_repo="$(mktemp -d /tmp/seguridad-rrhh.XXXXXX)" || return 1
  ...
  # Copia únicamente Seguridad-RRHH directamente en la ruta final.
...
install_app(){
  ...
  printf "\n${B}Construyendo y arrancando Seguridad RRHH...${N}\n"
...
install_offline(){
  title
  printf "${C}${B}Instalar Seguridad RRHH OFFLINE${N}\n\n"
  ...
  if [[ ! -f "$source_dir/docker-compose.yml" || ! -d "$source_dir/backend" || ! -d "$source_dir/frontend" ]]; then
    ...
    printf "${Y}%s${N}\n\n" "$source_dir"
    printf "Debes copiar este script dentro de la carpeta del proyecto Seguridad-RRHH,\n"
  ...
  printf "\n${B}Construyendo y arrancando Seguridad RRHH desde archivos locales...${N}\n"
...
update_app(){
  ...
  printf "\n${B}Reconstruyendo y reiniciando servicios sin borrar los datos...${N}\n"
  if dc up -d --build --remove-orphans; then
    printf "${G}Seguridad RRHH fue actualizado correctamente. La base de datos y configuración se conservaron.${N}\n"
    dc ps
    show_url
  else
...
while true; do
  ...
  printf "  ${Y}2)${C} Instalar o migrar a Seguridad RRHH (GitHub)${N}\n"
