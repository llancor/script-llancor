...
export async function validateShifts(guardiaId:string,candidates:Array<{fecha:Date;hora_inicio:string;hora_fin:string;horas_colacion?:number}>,excludeId?:string){
  ...
  const effective=(shift:any)=>Math.max(0,shift.hours-Math.max(0,Number(shift.horas_colacion)||0));
  let maxWeeklyHours=0;
  for(const candidate of candidates){const day=new Date(candidate.fecha),weekday=(day.getUTCDay()+6)%7,start=new Date(day);start.setUTCDate(day.getUTCDate()-weekday);start.setUTCHours(0,0,0,0);const end=new Date(start);end.setUTCDate(end.getUTCDate()+7);const weekly=all.filter(x=>x.start>=start&&x.start<end).reduce((sum,x)=>sum+effective(x),0);maxWeeklyHours=Math.max(maxWeeklyHours,weekly);}
