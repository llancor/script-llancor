...
const realtimeCreateFields:Record<string,string>={ronda:'fecha_hora_inicio',entrada:'hora_entrada',reporte:'fecha',alerta:'fecha'};
...
const protectedActions=['turno','ronda','entrada','reporte','alerta','relevo'];
...
const canTrackOwnRonda=(req:any)=>req.params.model==='ronda'&&req.user?.role==='guardia'&&req.user?.permisos?.rondas===true&&(!req.body?.estado||['En_curso','Completada'].includes(req.body.estado))&&Object.keys(req.body||{}).every(field=>['ruta','distancia_m','estado','fecha_hora_inicio','fecha_hora_fin'].includes(field));
...
crud.get('/:model',asyncHandler(async(req,res)=>{const model=req.params.model;const {q,page='1',limit='50',...filters}=req.query as any;const requested:any={};for(const [k,v] of Object.entries(filters))if(v)requested[k]=v;const searchable:Record<string,string[]>={guardia:['nombre','documento'],recinto:['nombre','direccion'],turno:['guardia_nombre','ubicacion'],ronda:['guardia_nombre','novedades'],entrada:['visitante_nombre','visitante_documento'],reporte:['titulo','descripcion'],alerta:['titulo','mensaje'],relevo:['guardia_saliente_nombre','guardia_entrante_nombre','novedades']};if(q)requested.OR=searchable[model].map(f=>({[f]:{contains:q}}));const where={AND:[requested,ownedWhere(req,model)]};const client=(db as any)[model];const relation=model==='alerta'?{include:{guardia:{select:{nombre:true}}}}:model==='guardia'?{include:{usuario:{select:{id:true,email:true,enabled:true}}}}:model==='relevo'?{include:{creador:{select:{id:true,full_name:true,guardia_id:true}}}}:{};const [rows,total]=await Promise.all([client.findMany({where,orderBy:{created_at:'desc'},skip:(+page-1)*(+limit),take:+limit,...relation}),client.count({where})]);const data=model==='alerta'?rows.map((item:any)=>({...item,guardia_nombre:item.guardia?.nombre||null})):rows;res.json({data,total,page:+page});}));
...
crud.post('/:model',asyncHandler(async(req,res)=>{
  ...
  if(req.params.model==='guardia'&&req.body.crear_acceso){
    ...
    const permisos={guardias:false,turnos:false,relevos:true,rondas:true,recintos:false,entradas:true,reportes:true,alertas:true,editar_relevos:false,eliminar_relevos:false,editar_rondas:false,eliminar_rondas:false,editar_entradas:false,eliminar_entradas:false,editar_reportes:false,eliminar_reportes:false,editar_alertas:false,eliminar_alertas:false,usuarios:false,configuracion:false,ver_registros:'propios'};
    ...
    if(req.body.enviar_invitacion)sendEmail(email,'Acceso creado en BastControl',`Hola ${item.nombre}. Tu cuenta fue creada. Usuario: ${email}\nContraseña temporal: ${password}\nPor seguridad deberás cambiarla al ingresar.`).catch(error=>console.error('No se pudo enviar la invitación:',error));
  ...
  const input={...req.body,created_by_id:req.user!.id};const realtimeField=realtimeCreateFields[req.params.model];if(realtimeField)input[realtimeField]=new Date();
  const data=normalize(req.params.model,input);const item=await (db as any)[req.params.model].create({data});req.app.get('io')?.emit(`${req.params.model}:created`,item);notifyCreated(req.params.model,item).catch(error=>console.error(`No se pudo notificar ${req.params.model}:`,error));res.status(201).json(item);
}));
crud.put('/:model/:id',asyncHandler(async(req,res)=>{if(!canEdit(req,req.params.model)&&!canTrackOwnRonda(req))return res.status(403).json({message:'No tienes permiso para editar registros de este módulo'});const current=await ensureVisible(req,res,true);if(!current)return;const input={...req.body};const realtimeField=realtimeCreateFields[req.params.model];if(realtimeField)delete input[realtimeField];if(req.params.model==='ronda'){delete input.fecha_hora_fin;if(current.estado!=='Completada'&&input.estado==='Completada')input.fecha_hora_fin=new Date();}if(req.user?.role==='guardia'&&protectedActions.includes(req.params.model)){delete input.guardia_id;delete input.guardia_nombre}if(req.params.model==='turno'){const guardiaId=input.guardia_id||current.guardia_id;if(guardiaId)await validateShifts(guardiaId,[{fecha:new Date(input.fecha||current.fecha),hora_inicio:input.hora_inicio||current.hora_inicio,hora_fin:input.hora_fin||current.hora_fin,horas_colacion:Number(input.horas_colacion??current.horas_colacion??0)}],req.params.id)}const data=normalize(req.params.model,input,true);delete data.created_by_id;const item=await (db as any)[req.params.model].update({where:{id:req.params.id},data});if(req.params.model==='guardia')await db.user.updateMany({where:{guardia_id:item.id},data:{full_name:item.nombre,telefono:item.telefono,rango:item.rango,foto_url:item.foto_url,enabled:item.estado==='Activo'}});req.app.get('io')?.emit(`${req.params.model}:updated`,item);res.json(item);}));
