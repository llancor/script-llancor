...
app.post('/api/config/test-email',auth,admin,asyncHandler(async(req,res)=>{if(!req.body.email)return res.status(400).json({message:'Indica un email de destino'});await sendEmail(req.body.email,'Prueba SMTP BastControl','La configuración SMTP funciona correctamente.');res.json({message:'Correo de prueba enviado'});}));
...
app.post('/api/turnos/programar',auth,permit('turnos'),manageTurnos,asyncHandler(async(req,res)=>{
  ...
  if(config?.shift_email_enabled&&guardia?.email)sendEmail(guardia.email,`Programación de turnos BastControl`,`${guardia_nombre}: ${dates.length} turno(s) desde ${fecha}, horario ${hora_inicio} a ${hora_fin}, ${recinto_nombre||ubicacion||'sin recinto indicado'}.`).catch(error=>console.error('No se pudo enviar programación:',error));
