...
app.put('/api/users/:id',auth,admin,asyncHandler(async(req,res)=>{
  const allowed=['full_name','email','role','rango','telefono','cargo','permisos','enabled'];
  const data:any=Object.fromEntries(Object.entries(req.body).filter(([key])=>allowed.includes(key)));
  const result=await db.$transaction(async transaction=>{
    const current=await transaction.user.findUnique({where:{id:req.params.id}});
    if(!current)return null;
    const removesActiveAdmin=current.role==='admin'&&current.enabled&&(data.enabled===false||('role' in data&&data.role!=='admin'));
    if(removesActiveAdmin&&await transaction.user.count({where:{role:'admin',enabled:true}})<=1){
      const error:any=new Error('No se puede desactivar ni cambiar el rol del último administrador activo');error.status=409;throw error;
    }
    const user=await transaction.user.update({where:{id:current.id},data});
    let revokedSessions=0;
    if(current.enabled&&user.enabled===false)revokedSessions=(await transaction.session.updateMany({where:{user_id:user.id,revoked_at:null},data:{revoked_at:new Date()}})).count;
    return{user,current,revokedSessions};
  },{isolationLevel:'Serializable'});
  if(!result)return res.status(404).json({message:'Usuario no encontrado'});
  const stateChanged=result.current.enabled!==result.user.enabled;
  await audit(stateChanged?(result.user.enabled?'usuario_activado':'usuario_desactivado'):'usuario_actualizado',{userId:req.user!.id,entity:'user',entityId:result.user.id,detail:{target_email:result.user.email,previous_role:result.current.role,role:result.user.role,previous_enabled:result.current.enabled,enabled:result.user.enabled,revoked_sessions:result.revokedSessions},ip:req.ip,userAgent:req.get('user-agent')});
...
app.delete('/api/users/:id',auth,admin,(req,res)=>res.status(405).json({message:'Las cuentas no se eliminan. Desactiva el usuario desde Usuarios'}));
