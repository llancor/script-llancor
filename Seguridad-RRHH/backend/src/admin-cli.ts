...
async function selectUser(selector?:string){
  if(!selector)throw new Error('Indica el número o email del usuario');
...
async function main(){
  ...
  if(command==='list-users'){
    const list=await users();console.table(list.map((u,index)=>({'#':index+1,email:u.email,nombre:u.full_name,rol:u.role,estado:u.enabled?'Activo':'Desactivado',bloqueo:u.locked_until&&u.locked_until>new Date()?`Hasta ${u.locked_until.toLocaleString('es-CL')}`:'No',intentos:u.failed_login_attempts})));return;
  }
  if(command==='reset-password'){
    if(!value||value.length<10)throw new Error('La contraseña debe tener al menos 10 caracteres');const user=await selectUser(selector);await db.user.update({where:{id:user.id},data:{password:await bcrypt.hash(value,12),email_verified:true,must_change_password:true,failed_login_attempts:0,locked_until:null}});console.log(`Contraseña actualizada y bloqueo retirado para ${user.email}`);return;
  }
  if(command==='unlock'){
