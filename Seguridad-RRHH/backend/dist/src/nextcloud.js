import { db, decryptSecret } from './lib.js';
const encodePath = (path) => path.split('/').filter(Boolean).map(encodeURIComponent).join('/');
async function settings() { const c = await db.configuracion.findUnique({ where: { id: 1 } }); if (!c?.nextcloud_enabled || !c.nextcloud_url || !c.nextcloud_user || !c.nextcloud_password)
...
} const match = file.dataUrl.match(/^data:([^;]+);base64,(.+)$/); if (!match)
    throw new Error('Archivo adjunto inválido'); const path = `${base}/${person}/${Date.now()}-${file.name.replace(/[^\p{L}\p{N}._-]+/gu, '_')}`, r = await fetch(`${root}/${encodePath(path)}`, { method: 'PUT', headers: { ...headers, 'Content-Type': match[1] }, body: Buffer.from(match[2], 'base64') }); if (!r.ok)
    throw new Error(`No se pudo subir a Nextcloud (${r.status})`); return path; }
