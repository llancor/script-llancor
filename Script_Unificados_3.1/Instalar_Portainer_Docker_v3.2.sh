...
header() {
    ...
    echo
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}        GESTOR DE PORTAINER             ${NC}"
    echo -e "${CYAN}========================================${NC}"
    echo
...
actualizar_portainer() {
    ...
    echo
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}      ACTUALIZAR PORTAINER              ${NC}"
    echo -e "${CYAN}========================================${NC}"
    echo

    # Verificar que Portainer exista
    if ! docker container inspect "$PORTAINER_NAME" >/dev/null 2>&1; then
        error "No se encontró una instalación de Portainer."
        pause
        return
    fi

    # Detectar la imagen utilizada actualmente
    PORTAINER_IMAGE=$(docker inspect \
        --format='{{.Config.Image}}' \
        "$PORTAINER_NAME" 2>/dev/null)

    if [ -z "$PORTAINER_IMAGE" ]; then
        warning "No fue posible detectar la imagen actual."
        PORTAINER_IMAGE="portainer/portainer-ce:lts"
        info "Se utilizará: $PORTAINER_IMAGE"
    else
        info "Imagen actual: $PORTAINER_IMAGE"
    fi

    echo

    backup_portainer || {
        error "No fue posible crear el respaldo."
        pause
        return
    }

    echo
    info "Descargando la última imagen..."

    if ! docker pull "$PORTAINER_IMAGE"; then
    ...
    if docker run -d \
        ...
        if docker ps --format '{{.Names}}' | grep -qx "$PORTAINER_NAME"; then
            ...
            success "Imagen: $PORTAINER_IMAGE"
        else
