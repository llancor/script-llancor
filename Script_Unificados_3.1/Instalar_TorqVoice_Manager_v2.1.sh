...
editar_env_torqvoice() {

    TORQVOICE_DIR="/opt/torqvoice"

    if [ ! -f "$TORQVOICE_DIR/.env" ]; then
        echo
    ...
    nano "$TORQVOICE_DIR/.env"

    echo
    read -rp "¿Desea reiniciar TorqVoice para aplicar los cambios? [S/n]: " RESP

    if [[ ! "$RESP" =~ ^[nN]$ ]]; then

        echo
        echo "🔄 Reiniciando TorqVoice..."

        cd "$TORQVOICE_DIR" || return 1

        if docker compose down && docker compose up -d; then
            echo
            echo "✅ TorqVoice reiniciado correctamente"
        else
            echo
            echo "❌ Error al reiniciar TorqVoice"
        fi
    fi
...
echo -e "${WHITE}1)${NC} ${YELLOW}Instalar TorqVoice${NC}"
echo -e "${WHITE}2)${NC} ${CYAN}Verificar dependencias${NC}"
...
echo -e "${WHITE}10)${NC} ${YELLOW}Editar .env de TorqVoice (URL / IP) Acceso${NC}"
echo
