...
const ownedWhere=(req:any,model:string,forceOwn=false)=>{const tenant=req.user?.empresa_id?{empresa_id:req.user.empresa_id}:{};if(!forceOwn&&canSeeAll(req))return tenant;const creator={created_by_id:req.user!.id};const guardiaId=req.user?.guardia_id;if(!guardiaId)return{AND:[tenant,creator]};if(model==='guardia')return{AND:[tenant,{OR:[creator,{id:guardiaId}]}]};if(model==='relevo')return{AND:[tenant,{OR:[creator,{guardia_saliente_id:guardiaId},{guardia_entrante_id:guardiaId}]}]};if(['turno','ronda','entrada','reporte','alerta'].includes(model))return{AND:[tenant,{OR:[creator,{guardia_id:guardiaId}]}]};return{AND:[tenant,creator]};};
...
crud.use('/:model',(req,res,next)=>{const permission=`${req.params.model}s`;if(req.user?.role!=='superadmin'&&req.user?.empresa_modulos?.[permission]===false)return res.status(403).json({message:'Este módulo no está habilitado para la empresa'});if(['admin','superadmin'].includes(req.user?.role||'')||req.user?.permisos?.[permission]===true)return next();return res.status(403).json({message:'No tienes permiso para acceder a este módulo'});});
...
crud.post('/:model',asyncHandler(async(req,res)=>{
  if(req.user!.empresa_id&&['guardia','recinto'].includes(req.params.model)){
    ...
    if(limit){const count=await(db as any)[req.params.model].count({where:{empresa_id:company.id}});if(count>=limit)return res.status(409).json({message:`La empresa alcanzó el límite de ${req.params.model==='guardia'?'guardias':'recintos'}`});}
  }
