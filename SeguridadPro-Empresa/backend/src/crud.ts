...
crud.use('/:model',(req,res,next)=>{const permission=`${req.params.model}s`;if(req.params.model==='turno'&&req.user?.role==='guardia'&&req.user?.guardia_id)return next();if(req.user?.role!=='superadmin'&&req.user?.empresa_modulos?.[permission]===false)return res.status(403).json({message:'Este módulo no está habilitado para la empresa'});if(['admin','superadmin'].includes(req.user?.role||'')||req.user?.permisos?.[permission]===true)return next();return res.status(403).json({message:'No tienes permiso para acceder a este módulo'});});
