...
$InstallRoot = if ($env:GUARDIAPRO_INSTALL_DIR) { $env:GUARDIAPRO_INSTALL_DIR } else { Join-Path $env:USERPROFILE 'seguridpro-rrhh' }
$ProjectSubdir = 'SeguridPro-HHRR'
...
function Write-Title {
    ...
    Write-Host '  SeguridPro RRHH - Administrador Windows' -ForegroundColor Cyan
    Write-Host '========================================' -ForegroundColor Cyan
    Write-Host
}
...
function Copy-ProjectToInstallRoot {
    ...
    if(Test-Path $envFile){
        $envBackup = Join-Path $env:TEMP ("seguridpro-env-" + [guid]::NewGuid().ToString('N') + '.bak')
...
function Get-Project {
    ...
    Write-Host 'Descargando SeguridPro RRHH desde GitHub...' -ForegroundColor Cyan
    $tempRoot = Join-Path $env:TEMP ("seguridpro-rrhh-" + [guid]::NewGuid().ToString('N'))
    try {
...
function Install-App {
    ...
    Write-Host 'Construyendo y arrancando SeguridPro RRHH...' -ForegroundColor Cyan;Invoke-Compose up -d --build
    if($LASTEXITCODE -eq 0){Write-Host 'SeguridPro RRHH instalado correctamente.' -ForegroundColor Green;Show-Url}else{Write-Host 'La instalación falló. Revisa los registros.' -ForegroundColor Red;Invoke-Compose logs --tail=100}
}
...
function Update-App {
    Write-Title
    Write-Host 'Actualizar SeguridPro RRHH' -ForegroundColor Cyan
    Write-Host
    ...
    if(-not (Project-Ready)){
        Write-Host "No se encontró SeguridPro RRHH en $InstallRoot." -ForegroundColor Red
        return
    }

    Write-Host 'Descargando únicamente SeguridPro-HHRR desde GitHub...' -ForegroundColor Cyan
    $tempRoot = Join-Path $env:TEMP ("seguridpro-rrhh-update-" + [guid]::NewGuid().ToString('N'))
    try {
        git clone --depth 1 --filter=blob:none --sparse --branch main $RepoUrl $tempRoot
        if($LASTEXITCODE -ne 0){Write-Host 'No fue posible descargar la actualización.' -ForegroundColor Red;return}
        git -C $tempRoot sparse-checkout set $ProjectSubdir
        if($LASTEXITCODE -ne 0){Write-Host 'No fue posible seleccionar la carpeta SeguridPro-HHRR.' -ForegroundColor Red;return}
    ...
    if($LASTEXITCODE -eq 0){Write-Host 'SeguridPro RRHH actualizado. La base de datos y el archivo .env se conservaron.' -ForegroundColor Green;Invoke-Compose ps;Show-Url}else{Write-Host 'La actualización falló. Revisa los registros.' -ForegroundColor Red}
}

while($true){Write-Title;Write-Host 'INSTALACIÓN' -ForegroundColor Cyan;Write-Option 1 'Instalar dependencias';Write-Option 2 'Instalar SeguridPro RRHH';Write-Host;Write-Host 'ADMINISTRACIÓN' -ForegroundColor Cyan;Write-Option 3 'Estado y control del servicio';Write-Option 4 'Cambiar puerto de Docker';Write-Option 5 'Ver URL y puerto';Write-Option 6 'Gestión de usuarios';Write-Host;Write-Host 'MANTENIMIENTO' -ForegroundColor Cyan;Write-Option 7 'Desinstalar';Write-Option 8 'Reparar base de datos';Write-Option 9 'Ver credenciales iniciales';Write-Option 10 'Actualizar SeguridPro RRHH';Write-Option 0 'Salir';switch(Read-Host 'Selecciona una opción'){'1'{Install-Dependencies;Pause-Menu}'2'{Install-App;Pause-Menu}'3'{Service-Menu}'4'{Change-Port;Pause-Menu}'5'{Write-Title;Show-Url;Pause-Menu}'6'{User-Menu}'7'{Uninstall-App;Pause-Menu}'8'{Repair-Database;Pause-Menu}'9'{Show-InitialCredentials;Pause-Menu}'10'{Update-App;Pause-Menu}'0'{exit}}}
