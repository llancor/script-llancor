...
migrate_legacy_env(){
  ...
  for legacy_env in \
    ...
    "/opt/seguridpro-rrhh/SeguridPro-HHRR/.env"; do
...
download_project(){
  ...
  temp_repo="$(mktemp -d /tmp/seguridpro-rrhh.XXXXXX)" || return 1
  ...
  # Copia únicamente SeguridPro-HHRR directamente en la ruta final.
...
while true; do
  ...
  printf "  ${Y}10)${Y} Actualizar SeguridPro RRHH${N}\n"
