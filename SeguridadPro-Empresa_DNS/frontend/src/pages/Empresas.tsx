...
export default function Empresas(){
  ...
  useEffect(()=>{load()},[]);
  const begin=(item?:any)=>{setEditing(item||null);setValue(item?{...blank(),...item,vence_at:item.vence_at?.slice(0,10)||'',modulos:{...blank().modulos,...item.modulos}}:blank());setOpen(true)};
  ...
  return <>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{items.map(item=><article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="my-4 grid grid-cols-3 gap-2 text-center text-sm"><Stat n={item._count.users} label="Usuarios"/><Stat n={item._count.guardias} label="Guardias"/><Stat n={item._count.recintos} label="Recintos"/></div>
...
function Stat({n,label}:{n:number;label:string}){return <div className="rounded-xl bg-slate-50 p-2 dark:bg-slate-950"><b className="block">{n}</b><span className="text-xs text-slate-500">{label}</span></div>}
