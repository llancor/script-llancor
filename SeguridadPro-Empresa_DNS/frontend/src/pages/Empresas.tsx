...
export default function Empresas(){
  ...
  return <>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{items.map(item=><article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="my-4 grid grid-cols-2 gap-2 text-center text-sm sm:grid-cols-3"><Stat n={`${item.uso?.usuarios_activos||0} activos · ${item.uso?.usuarios_creados||0} creados / ${item.limite_usuarios||'∞'}`} label="Usuarios"/><Stat n={`${item.uso?.guardias_activos||0} activos · ${item.uso?.guardias_creados||0} creados / ${item.limite_guardias||'∞'}`} label="Guardias"/><Stat n={item.vence_at?new Date(item.vence_at).toLocaleDateString('es-CL'):'Sin fecha'} label="Vencimiento"/></div>
