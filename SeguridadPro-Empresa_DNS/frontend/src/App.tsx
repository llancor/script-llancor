...
const Login=lazy(()=>import('./pages/Login')),Dashboard=lazy(()=>import('./pages/Dashboard')),CrudPage=lazy(()=>import('./pages/CrudPage')),Rondas=lazy(()=>import('./pages/Rondas')),Turnos=lazy(()=>import('./pages/Turnos')),Relevos=lazy(()=>import('./pages/Relevos')),Users=lazy(()=>import('./pages/Users')),Profile=lazy(()=>import('./pages/Profile')),Config=lazy(()=>import('./pages/Config')),PublicPage=lazy(()=>import('./pages/PublicPage')),RRHH=lazy(()=>import('./pages/RRHH')),Empresas=lazy(()=>import('./pages/Empresas')),Dominios=lazy(()=>import('./pages/Dominios')),MiEmpresa=lazy(()=>import('./pages/MiEmpresa'));
...
export default function App(){
  ...
  return <Layout><Suspense fallback={<Loading/>}><Routes>
    ...
    <Route path="/turnos" element={allowed('turnos')?<Turnos/>:<Navigate to="/dashboard"/>}/><Route path="/relevos" element={allowed('relevos')?<Relevos/>:<Navigate to="/dashboard"/>}/><Route path="/rondas" element={allowed('rondas')?<Rondas/>:<Navigate to="/dashboard"/>}/><Route path="/rrhh" element={allowed('rrhh')?<RRHH/>:<Navigate to="/dashboard"/>}/><Route path="/usuarios" element={['admin','superadmin'].includes(user?.role||'')?<Users/>:<Navigate to="/dashboard"/>}/><Route path="/empresas" element={user?.role==='superadmin'?<Empresas/>:<Navigate to="/dashboard"/>}/><Route path="/dominios" element={user?.role==='superadmin'?<Dominios/>:<Navigate to="/dashboard"/>}/><Route path="/mi-empresa" element={user?.role==='admin'?<MiEmpresa/>:<Navigate to="/dashboard"/>}/><Route path="/perfil" element={<Profile/>}/><Route path="/configuracion" element={user?.role==='superadmin'?<Config/>:<Navigate to="/dashboard"/>}/><Route path="/empresa" element={<PublicPage/>}/><Route path="*" element={<Navigate to="/dashboard"/>}/>
