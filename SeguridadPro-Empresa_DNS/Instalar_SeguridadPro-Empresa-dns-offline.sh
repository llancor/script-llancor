...
REPO_URL="${SEGURIDAD_REPO_URL:-https://github.com/llancor/script-llancor.git}"
...
if [[ -z "${SEGURIDAD_INSTALL_DIR:-}" && -f "$SCRIPT_DIR/docker-compose.yml" ]]; then
...
title(){
  clear
  printf "${C}${B}==================================================\n"
  printf "           SeguridadPro-Empresa DNS v5.1          \n"
...
migrate_legacy_env(){
  ...
  for legacy_env in \
    "/opt/seguridpro/seguridad-rrhh/.env" \
    "/opt/seguridpro/seguridpro/.env" \
    "/opt/seguridad-rrhh/Seguridad-RRHH/.env"; do
...
download_project(){
  ...
  title
  printf "${B}Descargando SeguridadPro-Empresa DNS v5.1 desde GitHub...${N}\n\n"
  ...
  temp_repo="$(mktemp -d /tmp/seguridadpro-empresa-dns.XXXXXX)" || return 1
  ...
  # Copia únicamente SeguridadPro-Empresa_DNS_v5.1 en la ruta final.
  ...
  if [[ ! -f .env ]]; then
    if [[ -f .env.docker.example ]]; then
      ...
      printf '%s\n' \
        ...
        'MYSQL_ROOT_PASSWORD=' \
        'JWT_SECRET=' \
        'INITIAL_ADMIN_EMAIL=admin@seguridad.cl' \
        ...
        'SMTP_USER=' \
        'SMTP_PASS=' \
        'MAIL_FROM=Seguridad <no-reply@seguridad.local>' > .env
    ...
    setenv INITIAL_ADMIN_EMAIL "admin@seguridad.cl"
...
install_app(){
  ...
  printf "\n${B}Construyendo y arrancando SeguridadPro-Empresa DNS v5.1...${N}\n"
...
install_offline(){
  title
  printf "${C}${B}Instalar SeguridadPro-Empresa DNS v5.1 OFFLINE${N}\n\n"

  local source_dir="$SCRIPT_DIR"
  local chosen port url
  ...
  if [[ ! -f "$source_dir/docker-compose.yml" || ! -d "$source_dir/backend" || ! -d "$source_dir/frontend" ]]; then
    ...
    printf "${Y}%s${N}\n\n" "$source_dir"
    printf "Debes copiar este script dentro de la carpeta del proyecto SeguridadPro-Empresa_DNS_v5.1,\n"
    printf "junto a docker-compose.yml, backend/ y frontend/.\n"
    return 1
  fi
  ...
  printf "Ruta de instalación [${C}%s${N}]: " "$DEFAULT_INSTALL_ROOT"
  ...
  if ! [[ "$port" =~ ^[0-9]+$ ]] || ((port < 1 || port > 65535)); then
    printf "${R}Puerto inválido. Debe estar entre 1 y 65535.${N}\n"
    return 1
  fi
  preflight "$chosen" "$port" || return 1
  ...
  printf "\n${B}Copiando proyecto local a %s...${N}\n" "$INSTALL_ROOT"
  if ! rsync -a --delete \
      --exclude='.env' \
      --exclude="$(basename "$0")" \
      "$source_dir/" "$INSTALL_ROOT/"; then
    [[ -n "$backup_env" ]] && rm -f -- "$backup_env"
    printf "${R}No fue posible copiar el proyecto local.${N}\n"
    return 1
  fi
  ...
  INSTALL_HTTP_PORT="$port"
  setenv HTTP_PORT "$port"
  setenv COMPOSE_PROJECT_NAME "$(instance_name "$INSTALL_ROOT")"

  printf "URL actual: ${C}%s${N}\n" "$(getenv APP_URL '')"
  read -rp 'URL pública (Enter para conservar): ' url </dev/tty
  [[ -n "$url" ]] && setenv APP_URL "$url"

  printf "\n${B}Construyendo y arrancando SeguridadPro-Empresa DNS v5.1 desde archivos locales...${N}\n"

  if dc up -d --build; then
    printf "${G}Instalación OFFLINE terminada correctamente.${N}\n"
    show_url
    print_initial_credentials
  else
    printf "${R}Falló la instalación offline. Revisa los registros desde el menú de estado.${N}\n"
    return 1
  fi
}
...
update_offline(){
  title
  printf "${C}${B}Actualizar SeguridadPro-Empresa DNS v5.1 OFFLINE${N}\n\n"
...
print_initial_credentials(){
  ...
  printf "${C}${B}==================================================${N}\n\n"
  printf "${B}Usuario:${N}     %s\n" "$(getenv INITIAL_ADMIN_EMAIL 'admin@seguridad.cl')"
...
restore_app(){
  local backup_dir="${SEGURIDAD_BACKUP_DIR:-${INSTALL_ROOT}-backups}" file confirmation
  find "$backup_dir" -maxdepth 1 -type f -name '*.sql.gz' -printf '%p\n' 2>/dev/null | sort -r | head -20
  read -rp 'Ruta completa del respaldo .sql.gz: ' file; [[ -f "$file" ]] || { printf "${R}Respaldo no encontrado.${N}\n"; return 1; }
...
update_app(){
  title
  printf "${C}${B}Actualizar SeguridadPro-Empresa DNS v5.1 desde GitHub${N}\n\n"
  ...
  printf "\n${B}Reconstruyendo y reiniciando servicios sin borrar los datos...${N}\n"
  if dc up -d --build --remove-orphans; then
    printf "${G}SeguridadPro-Empresa DNS v5.1 fue actualizado correctamente. La base de datos y configuración se conservaron.${N}\n"
    dc ps
    show_url
  else
...
uninstall_app(){
  title
  printf "${C}${B}Desinstalacion completa de SeguridadPro-Empresa DNS v5.1${N}\n\n"
...
while true; do
  ...
  printf "  ${Y}2)${C} Instalar o migrar a SeguridadPro-Empresa DNS v5.1 (GitHub)${N}\n"
  printf "  ${Y}3)${Y} Instalar offline desde esta carpeta${N}\n\n"
  printf "${C}${G} ADMINISTRACIÓN${N}\n"
  printf "  ${Y}4)${C} Estado y control del servicio${N}\n"
  printf "  ${Y}5)${C} Cambiar puerto de Docker${N}\n"
  printf "  ${Y}6)${C} Ver URL y puerto${N}\n"
  printf "  ${Y}7)${C} Gestión de usuarios${N}\n\n"
  printf "${C}${G} MANTENIMIENTO${N}\n"
  printf "  ${Y}8)${C} Desinstalar y borrar todo${N}\n"
  printf "  ${Y}9)${C} Reparar base de datos${N}\n"
  printf "  ${Y}10)${Y} Ver credenciales iniciales${N}\n"
  printf "  ${Y}11)${Y} Actualizar SeguridadPro-Empresa DNS v5.1 desde GitHub${N}\n"
  ...
  case "$o" in
    1) dependencies; pause;;
    2) install_app; pause;;
    3) install_offline; pause;;
    4) services;;
    5) port; pause;;
    6) title; show_url; pause;;
    7) users;;
    8) uninstall_app; pause;;
    9) repair_database; pause;;
    10) show_initial_credentials; pause;;
    11) update_app; pause;;
    12) backup_app; pause;;
