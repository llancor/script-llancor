...
import { dominios,resolverDominio } from './dominios.js';
...
app.get('/api/public/domain/resolve',resolverDominio);
...
app.get('/api/dashboard',auth,asyncHandler(async(req,res)=>{
  if(req.user?.role==='superadmin')return res.status(403).json({message:'El propietario de la plataforma no tiene acceso a información operativa privada de las empresas'});
...
app.use('/api/dominios',auth,dominios);
...
app.delete('/api/users/:id',auth,admin,asyncHandler(async(req,res)=>{
  if(req.params.id===req.user!.id)return res.status(400).json({message:'No puedes eliminar tu usuario desde administración'});
...
app.post('/api/config/test-email',auth,superadmin,asyncHandler(async(req,res)=>{if(!req.body.email)return res.status(400).json({message:'Indica un email de destino'});await sendEmail(req.body.email,'Prueba SMTP Seguridad-RRHH','La configuración SMTP funciona correctamente.');res.json({message:'Correo de prueba enviado'});}));
app.post('/api/config/test-telegram',auth,superadmin,asyncHandler(async(req,res)=>{await sendTelegram('La configuración de Telegram funciona correctamente.');res.json({message:'Mensaje de prueba enviado'});}));
...
async function migrateCompatibility(){
  ...
  await db.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS empresa_dominios (
    id VARCHAR(30) PRIMARY KEY,empresa_id VARCHAR(30) NOT NULL,dominio VARCHAR(253) NOT NULL UNIQUE,
    tipo ENUM('Dominio','Subdominio','SubdominioSistema') NOT NULL DEFAULT 'Dominio',principal BOOLEAN NOT NULL DEFAULT FALSE,
    estado ENUM('Pendiente','Verificando','Activo','Error_DNS','Error_SSL','Suspendido') NOT NULL DEFAULT 'Pendiente',
    token_verificacion VARCHAR(191) NOT NULL,destino_esperado VARCHAR(253) NULL,verificado_at DATETIME NULL,
    ssl_estado VARCHAR(191) NOT NULL DEFAULT 'Pendiente',ssl_error TEXT NULL,ultimo_error TEXT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    KEY idx_empresa_dominios_empresa_estado (empresa_id,estado),CONSTRAINT fk_empresa_dominios_empresa FOREIGN KEY (empresa_id) REFERENCES empresas(id) ON DELETE CASCADE
  ) ENGINE=InnoDB`);
  ...
  // Todas las tablas deben existir antes de aplicarles la separación por empresa.
