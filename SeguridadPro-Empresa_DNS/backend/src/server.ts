...
app.delete('/api/users/:id',auth,admin,asyncHandler(async(req,res)=>{if(req.params.id===req.user!.id)return res.status(400).json({message:'No puedes eliminar tu usuario desde administración'});const current=await db.user.findFirst({where:{id:req.params.id,...(req.user!.role==='superadmin'?{}:{empresa_id:req.user!.empresa_id})}});if(!current)return res.status(404).json({message:'Usuario no encontrado en esta empresa'});await db.$transaction(async tx=>{await tx.trabajador.updateMany({where:{user_id:current.id},data:{user_id:null}});await tx.user.delete({where:{id:current.id}})});res.status(204).end();}));
