...
const editable=z.object({nombre:z.string().min(2).max(191),rut:z.string().max(191).optional().nullable(),email:z.string().email().optional().nullable(),telefono:z.string().max(191).optional().nullable(),direccion:z.string().max(255).optional().nullable(),logo_url:z.string().optional().nullable(),website_url:z.string().url().optional().nullable().or(z.literal('')),descripcion:z.string().max(10000).optional().nullable(),servicios:z.string().max(10000).optional().nullable(),quote_email:z.string().email().optional().nullable(),public_page_enabled:z.boolean().optional()});

miEmpresa.use(companyAdmin);
miEmpresa.get('/',asyncHandler(async(req,res)=>{const item=await db.empresa.findUniqueOrThrow({where:{id:req.user!.empresa_id},include:{dominios:{orderBy:[{principal:'desc'},{dominio:'asc'}]},_count:{select:{users:true,guardias:true,recintos:true,turnos:true}}}});res.json(item);}));
