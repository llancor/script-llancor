...
install_dependencies() {
    ...
    apt-get update
    PACKAGES=(apache2 mariadb-server mariadb-client php php-mysql php-cli php-curl php-gd php-xml php-mbstring php-zip unzip wget curl zip)
    for pkg in "${PACKAGES[@]}"; do
        if dpkg -s "$pkg" &>/dev/null; then
            echo -e "${GREEN}$pkg ya instalado.${RESET}"
        else
...
echo -e "${YELLOW}4)${RESET} ${CYAN}Salir${RESET}"
