...
DEFAULT_INSTALL_ROOT="/opt/SeguridadPro-Empresa"
DEFAULT_HTTP_PORT="8082"
INSTALL_ROOT="${SEGURIDAD_INSTALL_DIR:-$DEFAULT_INSTALL_ROOT}"
PROJECT_SUBDIR="SeguridadPro-Empresa"
APP_DIR="$SCRIPT_DIR"
...
download_project(){
  ...
  title
  printf "${B}Descargando SeguridadPro-Empresa desde GitHub...${N}\n\n"
  ...
  temp_repo="$(mktemp -d /tmp/seguridadpro-empresa.XXXXXX)" || return 1
  ...
  # Copia únicamente SeguridadPro-Empresa directamente en la ruta final.
...
install_app(){
  ...
  printf "\n${B}Construyendo y arrancando SeguridadPro-Empresa...${N}\n"
...
install_offline(){
  title
  printf "${C}${B}Instalar SeguridadPro-Empresa OFFLINE${N}\n\n"
  ...
  if [[ ! -f "$source_dir/docker-compose.yml" || ! -d "$source_dir/backend" || ! -d "$source_dir/frontend" ]]; then
    ...
    printf "${Y}%s${N}\n\n" "$source_dir"
    printf "Debes copiar este script dentro de la carpeta del proyecto SeguridadPro-Empresa,\n"
  ...
  printf "\n${B}Construyendo y arrancando SeguridadPro-Empresa desde archivos locales...${N}\n"
...
update_offline(){
  title
  printf "${C}${B}Actualizar SeguridadPro-Empresa OFFLINE${N}\n\n"
...
update_app(){
  title
  printf "${C}${B}Actualizar SeguridadPro-Empresa desde GitHub${N}\n\n"
  ...
  printf "\n${B}Reconstruyendo y reiniciando servicios sin borrar los datos...${N}\n"
  if dc up -d --build --remove-orphans; then
    printf "${G}SeguridadPro-Empresa fue actualizado correctamente. La base de datos y configuración se conservaron.${N}\n"
    dc ps
    show_url
  else
...
uninstall_app(){
  title
  printf "${C}${B}Desinstalacion completa de SeguridadPro-Empresa${N}\n\n"
...
while true; do
  ...
  printf "  ${Y}2)${C} Instalar o migrar a SeguridadPro-Empresa (GitHub)${N}\n"
  ...
  printf "  ${Y}11)${Y} Actualizar SeguridadPro-Empresa desde GitHub${N}\n"
