...
cambiar_superusuario(){
  ...
  local cid sj actual nuevo pass pass2
  ...
  docker exec -e SJ="$sj" "$cid" node -e 'const fs=require("fs");const a=JSON.parse(fs.readFileSync(process.env.SJ));(Array.isArray(a)?a:[a]).forEach((u,i)=>console.log(`${i+1}) ${u.mail||u.email||""}`));'
  echo
  read -rp "Correo del superusuario a modificar [$DEFAULT_SUPER_USER]: " actual; actual=${actual:-$DEFAULT_SUPER_USER}
  read -rp "Nuevo correo [mantener $actual]: " nuevo; nuevo=${nuevo:-$actual}
...
const fs=require("fs"),crypto=require("crypto");const p=process.env.SJ;let a=JSON.parse(fs.readFileSync(p));if(!Array.isArray(a))a=[a];const i=a.findIndex(u=>(u.mail||u.email)===process.env.OLD_MAIL);if(i<0)process.exit(4);a[i].mail=process.env.NEW_MAIL;delete a[i].email;a[i].pass=crypto.createHash("sha256").update(process.env.NEW_PASS).digest("hex");fs.writeFileSync(p,JSON.stringify(a,null,3));'; then
    printf "Usuario: %s\nContraseña: %s\n" "$nuevo" "$pass" > "$CRED_FILE"; chmod 600 "$CRED_FILE" 2>/dev/null || true
    docker restart "$cid" >/dev/null
    echo -e "${GREEN}✓ Superusuario actualizado y Shinobi reiniciado.${NC}"
    echo "Usuario: $nuevo"
    echo "Contraseña: $pass"
  else
    echo -e "${RED}No fue posible modificar el superusuario. Verifique el correo seleccionado.${NC}"
  fi
  pause
}
