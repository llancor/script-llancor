...
backup_app(){
  ...
  cp -p .env "$backup_dir/seguridad-$stamp.env"; chmod 600 "$file" "$backup_dir/seguridad-$stamp.env"; printf "${G}Respaldo completado: %s${N}\n" "$file"
}
...
while true; do
  ...
  printf "  ${Y}2)${C} Instalar o migrar a SeguridadPro-Empresa DNS v5.1${N}\n\n"
  ...
  printf "  ${Y}10)${Y} Actualizar SeguridadPro-Empresa DNS v5.1${N}\n"
