...
DESTINO="/root"
TOKEN_FILE="/root/token-github"

# Autenticación opcional GitHub
obtener_token_github() {
    GITHUB_TOKEN=""
    ...
    echo
    echo -e "${YELLOW}GitHub puede limitar las descargas anónimas (HTTP 429).${NC}"
    echo "[1] Ingresar token y guardarlo en $TOKEN_FILE"
    echo "[2] Continuar sin token"
    read -rp "Seleccione una opción [1]: " TOKEN_OPCION

    case "${TOKEN_OPCION:-1}" in
        1)
            ...
            if [ -n "$GITHUB_TOKEN" ]; then
                ...
                echo -e "${GREEN}✅ Token guardado en $TOKEN_FILE (root:root, permisos 600)${NC}"
            fi
            ;;
        *) GITHUB_TOKEN="" ;;
    esac
...
obtener_scripts() {
    ...
    mapfile -t URLS < <(
        curl_github -s "https://api.github.com/repos/${REPO}/contents/${RUTA_REPO}" |
        grep '"download_url"' |
        cut -d '"' -f4 |
        grep -Ei '\.(sh|bash)$'
    )
...
descargar_script() {
    ...
    if curl_github -L --fail --progress-bar -o "$RUTA_LOCAL" "$URL"; then
...
actualizar_script() {
    ...
    if curl_github -L --fail --progress-bar -o "$DESTINO/$ARCHIVO" "$URL"; then
...
# ----------------------------------------------------------
configurar_token_github() {
    echo
    echo -e "${CYAN}══════════════════════════════════════════════${NC}"
    echo -e "${CYAN}          TOKEN DE GITHUB${NC}"
    echo -e "${CYAN}══════════════════════════════════════════════${NC}"
    echo
    if [ -s "$TOKEN_FILE" ]; then
        echo -e "${GREEN}✅ Existe token guardado en $TOKEN_FILE${NC}"
    else
        echo -e "${YELLOW}⚠️ No existe token guardado.${NC}"
    fi
    echo
    echo "[1] Ingresar y guardar token"
    echo "[2] Eliminar token guardado"
    echo "[0] Volver"
    read -rp "Seleccione una opción: " T
    case "$T" in
        1)
            read -rsp "Token GitHub: " TOKEN_NUEVO
            echo
            if [ -n "$TOKEN_NUEVO" ]; then
                printf '%s\n' "$TOKEN_NUEVO" > "$TOKEN_FILE"
                chmod 600 "$TOKEN_FILE"
                chown root:root "$TOKEN_FILE" 2>/dev/null || true
                echo -e "${GREEN}✅ Token guardado correctamente en $TOKEN_FILE.${NC}"
            fi
            ;;
        2)
            rm -f "$TOKEN_FILE"
...
echo -e "${YELLOW}[9]${NC} Configurar token GitHub"
echo -e "${CYAN}[0]${NC} Salir"
echo
    ...
    case "$MENU" in
        ...
        9) configurar_token_github; read -n1 -s -r -p "Presione una tecla..." ;;
        0) exit 0 ;;
