...
menu_servicio_ups() {
    ...
    do
        ...
        case "$OPCION" in
            ...
            1) crear_servicio_ups ;;

            2) activar_monitor_ups ;;

            3) desactivar_monitor_ups ;;

            4) estado_monitor_ups ;;
...
crear_servicio_ups() {
    ...
    SCRIPT_ORIGINAL="/root/Gestion_UPS_APC-SRV3KI_Cliente_LAN_v4.3.2.sh"
    ...
    if systemctl list-unit-files | grep -q "^ups-apc-monitor.service"
    then

        echo
        echo "Deteniendo servicio existente..."

        systemctl stop ups-apc-monitor.service 2>/dev/null
    ...
    cp "$SCRIPT_ORIGINAL" "$SCRIPT_SERVICIO"
...
[Service]
Type=simple
ExecStart=/bin/bash $SCRIPT_SERVICIO --service
...
EOF
    ...
    systemctl enable ups-apc-monitor.service
    ...
    systemctl start ups-apc-monitor.service
    ...
    then
        ...
        systemctl status ups-apc-monitor.service --no-pager
    ...
    then
        ...
        echo "$SCRIPT_SERVICIO"
        echo
        echo "Servicio:"
        echo "$SERVICIO"
        echo
        echo "Ejecutando:"
        echo "$SCRIPT_SERVICIO --service"
        echo
        echo "Inicio automático: ACTIVADO"
        echo
        echo "Estado:"
        systemctl is-active ups-apc-monitor.service
        echo
...
desactivar_monitor_ups() {
    ...
    systemctl stop ups-apc-monitor.service

    systemctl disable ups-apc-monitor.service
...
############################################################
# ESTADO SERVICIO UPS APC MONITOR
############################################################
...
# Alias: el servicio de monitoreo real (versión USB que funciona
# al 100%) llama a "apagar_host_proxmox", no a "apagar_host".
# Se mantiene un solo cuerpo de función para no duplicar lógica.
apagar_host_proxmox() {
    apagar_host
}
############################################################
...
servicio_apagado_ups() {
    ...
    # Determina si este equipo es servidor NUT (USB local) o
    # cliente NUT (UPS remota por LAN), y fija UPS_OBJETIVO y
    # SERVICIO_NUT_LOCAL en consecuencia.
    ...
    then
        ...
        determinar_ups_objetivo   # fija UPS_REMOTA leyendo upsmon.conf
    ...
    # obtener_valor(), dibujar_panel() y actualizar_panel() usan la
    # variable global UPS_REMOTA en ambos modos; se sincroniza aquí
    # para que el panel funcione igual en servidor y en cliente.
    ...
    do
        ...
        then
            ...
            then
                ...
                if [[ "$MODO_NUT" == "netclient" ]]
                then
                    echo "  - Servidor NUT apagado o inalcanzable por red"
                    echo "  - Puerto 3493 bloqueado por firewall"
                    echo "  - Usuario/contraseña incorrectos en upsmon.conf"
                    echo "  - Nombre de UPS incorrecto en la cadena UPS@host"
                else
        ...
        then
            ...
            registrar_log \
            "Estado:$ESTADO Bateria:$BATERIA Autonomia:$AUTONOMIA"
