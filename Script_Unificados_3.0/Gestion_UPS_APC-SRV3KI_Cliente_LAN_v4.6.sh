...
SCRIPT="/root/Gestion_UPS_APC-SRV3KI_Cliente_LAN_v4.6.sh"
...
verificar_apagado_automatico() {
    ...
    if [[ "${MONITOR_APAGADO^^}" == "ON" ]]; then
        ...
        echo -e "${ERROR} Apagado automático       : ${ROJO}DESACTIVADO${RESET} (MONITOR_APAGADO=${MONITOR_APAGADO:-NO DEFINIDO})"
        ((ERRORES++))
    fi
    ...
    if (( ERRORES == 0 )); then
        ...
        registrar_log "Verificación apagado automático: OK"
    else
