...
DEFAULT_INSTALL_ROOT="/opt/guardiapro-rrhh"
DEFAULT_HTTP_PORT="8082"
INSTALL_ROOT="${GUARDIAPRO_INSTALL_DIR:-$DEFAULT_INSTALL_ROOT}"
PROJECT_SUBDIR="guardiapro-rrhh"
LEGACY_PROJECT_SUBDIR="guardiapro"
APP_DIR="$SCRIPT_DIR"
...
migrate_legacy_env(){
  local legacy_env="$INSTALL_ROOT/$LEGACY_PROJECT_SUBDIR/.env"
  local target_env="$INSTALL_ROOT/.env"
  if [[ ! -f "$target_env" && -f "$legacy_env" ]]; then
...
download_project(){
  if project_ready; then
    printf "${G}Proyecto ya disponible en $APP_DIR.${N}\n"
    return 0
  fi
  title
  ...
  local tmp_dir backup_env
  tmp_dir="$(mktemp -d /tmp/guardiapro-rrhh.XXXXXX)" || return 1
  backup_env=""
  if [[ -f "$INSTALL_ROOT/.env" ]]; then
    backup_env="$(mktemp /tmp/guardiapro-env.XXXXXX)" || { rm -rf "$tmp_dir"; return 1; }
    cp "$INSTALL_ROOT/.env" "$backup_env" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  fi

  GIT_TERMINAL_PROMPT=0 git clone --depth 1 --filter=blob:none --sparse --branch main "$REPO_URL" "$tmp_dir/repo" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  git -C "$tmp_dir/repo" sparse-checkout set "$PROJECT_SUBDIR" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  [[ -f "$tmp_dir/repo/$PROJECT_SUBDIR/docker-compose.yml" ]] || { printf "${R}No se encontró $PROJECT_SUBDIR/docker-compose.yml en GitHub.${N}\n"; rm -rf "$tmp_dir" "$backup_env"; return 1; }

  root install -d -o "$(id -un)" -g "$(id -gn)" "$INSTALL_ROOT" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  find "$INSTALL_ROOT" -mindepth 1 -maxdepth 1 ! -name '.env' -exec rm -rf -- {} +
  cp -a "$tmp_dir/repo/$PROJECT_SUBDIR/." "$INSTALL_ROOT/" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  if [[ -n "$backup_env" && -f "$backup_env" ]]; then cp "$backup_env" "$INSTALL_ROOT/.env"; fi
  rm -rf "$tmp_dir" "$backup_env"
  APP_DIR="$INSTALL_ROOT"
  cd "$APP_DIR" || return 1
  migrate_legacy_env || return 1
  printf "${G}Proyecto instalado directamente en $APP_DIR.${N}\n"
}
setenv(){
...
install_app(){
  ...
  if dc up -d --build; then printf "${G}Instalación terminada.${N}\n"; show_url; printf "\n${C}${B}Credenciales iniciales${N}\n\n${B}Usuario:${N} admin@guardiapro.cl\n${B}Contraseña:${N} GuardiaPro2026!\n\n${Y}Por seguridad, cambia esta contraseña después del primer inicio de sesión.${N}\n"; else printf "${R}Falló la instalación. Revisa los registros desde el menú de estado.${N}\n"; fi
}
...
update_app(){
  ...
  project_ready || { printf "${R}No se encontró una instalación válida en $INSTALL_ROOT.${N}\n"; return 1; }
  has git || { printf "${R}Git no está instalado. Ejecuta primero la opción 1.${N}\n"; return 1; }

  local tmp_dir backup_env
  tmp_dir="$(mktemp -d /tmp/guardiapro-update.XXXXXX)" || return 1
  backup_env="$(mktemp /tmp/guardiapro-env.XXXXXX)" || { rm -rf "$tmp_dir"; return 1; }
  cp "$INSTALL_ROOT/.env" "$backup_env" 2>/dev/null || :

  printf "${B}Descargando únicamente $PROJECT_SUBDIR desde GitHub...${N}\n"
  GIT_TERMINAL_PROMPT=0 git clone --depth 1 --filter=blob:none --sparse --branch main "$REPO_URL" "$tmp_dir/repo" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  git -C "$tmp_dir/repo" sparse-checkout set "$PROJECT_SUBDIR" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  [[ -f "$tmp_dir/repo/$PROJECT_SUBDIR/docker-compose.yml" ]] || { printf "${R}La carpeta descargada no contiene una instalación válida.${N}\n"; rm -rf "$tmp_dir" "$backup_env"; return 1; }

  find "$INSTALL_ROOT" -mindepth 1 -maxdepth 1 ! -name '.env' -exec rm -rf -- {} +
  cp -a "$tmp_dir/repo/$PROJECT_SUBDIR/." "$INSTALL_ROOT/" || { rm -rf "$tmp_dir" "$backup_env"; return 1; }
  [[ -s "$backup_env" ]] && cp "$backup_env" "$INSTALL_ROOT/.env"
  rm -rf "$tmp_dir" "$backup_env"
  APP_DIR="$INSTALL_ROOT"
  cd "$APP_DIR" || return 1
  prepare_env
...
while true; do
  ...
  printf "  ${Y}10)${Y} Actualizar GuardiaPro RRHH${N}\n"
