...
actualizar_panel() {
    ...
    tput cup 35 0
    tput el
    printf "${CYAN}Monitor automático :${RESET} %b" "$ESTADO_MONITOR"
...
EOF
    ...
    mensaje_ok "BOT configurado correctamente."
...
############################################################
# CREAR SERVICIO SYSTEMD UPS APC MONITOR
############################################################
...
crear_servicio_ups() {
    ...
    ORIGEN="/root/Gestion_UPS_APC-SRV3KI_v4.2.sh"
    SCRIPT="/usr/local/bin/ups-apc-monitor.sh"
    SERVICE="/etc/systemd/system/ups-apc-monitor.service"


    if [[ ! -f "$ORIGEN" ]]
    then

        mensaje_error "No existe el script origen:"
        echo "$ORIGEN"
...
cat > "$SERVICE" <<EOF
[Unit]
Description=APC UPS Automatic Shutdown Monitor
...
RestartSec=10
User=root
StandardOutput=append:/var/log/ups-apc-monitor.log
StandardError=append:/var/log/ups-apc-monitor.log
...
EOF
    ...
    mensaje_ok "Servicio UPS APC creado correctamente."
...
menu() {
    ...
    do
		...
		echo
		echo -e "${VERDE}15)${RESET} Configurar Telegram"
        echo -e "${VERDE}16)${RESET} Probar Telegram"
		echo -e "${VERDE}17)${RESET} Activar/desactivar alertas Telegram"
		echo
		echo -e "${VERDE}18)${AMARILLO} Menu Configurar Correo ${VERDE}SMTP / Alertas"
		echo
		echo -e "${VERDE}19)${RESET} Estado general del sistema"
		echo
		echo -e "${VERDE}20)${AMARILLO} Menu Servicio systemd UPS APC ${VERDE}* Activa Mensajes Automaticos"
		echo
