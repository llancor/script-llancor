...
actualizar_npm() {
    ...
    header "ACTUALIZANDO NGINX PROXY MANAGER"

    if [ ! -d "$NPM_DIR" ]; then

        echo
        echo -e "${RED}Nginx Proxy Manager no está instalado.${NC}"
        pause
        return
    ...
    echo
    echo -e "${CYAN}Creando respaldo rápido...${NC}"

    FECHA=$(date +%Y%m%d-%H%M%S)

    tar czf \
        "/tmp/npm-backup-${FECHA}.tar.gz" \
        data letsencrypt 2>/dev/null

    echo
    echo -e "${CYAN}Descargando última versión...${NC}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo
        echo -e "${RED}Error descargando la imagen.${NC}"
        pause
        return
    ...
    echo
    echo -e "${CYAN}Recreando contenedor...${NC}"
    ...
    if docker ps --format '{{.Names}}' | grep -q '^nginx-proxy-manager$'; then
        ...
        echo
        echo -e "${GREEN}✔ Nginx Proxy Manager actualizado correctamente${NC}"
        echo

        docker ps --filter "name=nginx-proxy-manager"

        echo
        echo -e "${CYAN}Imagen actual:${NC}"
        ...
        echo
        echo -e "${RED}✘ Error al iniciar Nginx Proxy Manager${NC}"
        echo
...
echo -e "${YELLOW}1)${CYAN} Instalar Nginx Proxy Manager."
echo -e "${YELLOW}2)${CYAN} Actualizar Nginx Proxy Manager."
echo -e "${YELLOW}3)${RED} Desinstalar"
echo -e "${YELLOW}4)${RESET} Ver estado"
echo -e "${YELLOW}5)${RESET} Reiniciar"
echo -e "${YELLOW}6)${RESET} Bloquear puerto 80"
echo -e "${YELLOW}7)${RESET} Desbloquear puerto 80"
echo -e "${YELLOW}8)${RESET} Estado puerto 80"
echo -e "${YELLOW}9)${CYAN} Respadar/Restaurar Ajustes Nginx"
echo -e "${YELLOW}10)${GREEN} 📤 Exportar migración Docker NPM"
echo -e "${YELLOW}11)${RED} 📥 Importar migración Docker NPM"
echo -e "${YELLOW}0)${RESET} Volver"
...
header "* INSTALADOR Y GESTOR NGINX v3.2 / SISTEMA DE RESPALDO *"
