...
# =========================================================
# LIMPIEZA ALMACENAMIENTO NEXTCLOUD
# Papelera / Versiones / Uploads temporales
# =========================================================
menu_limpieza_nextcloud(){
    ...
    # Suma el tamaño real de todas las carpetas de una categoría.
    _nc_size_categoria(){
        local nombre="$1"
        local total=0 tam dir

        while IFS= read -r -d '' dir; do
            tam=$(du -sb -- "$dir" 2>/dev/null | awk '{print $1}')
            [[ "$tam" =~ ^[0-9]+$ ]] || tam=0
            total=$((total + tam))
        done < <(find "$DATA_DIR_LIMPIEZA" -mindepth 2 -maxdepth 2 \
                     -type d -name "$nombre" -print0 2>/dev/null)

        echo "$total"
    }

    # Muestra el detalle por usuario/carpeta.
    _nc_detalle_categoria(){
        local nombre="$1"
        local encontrados=0 dir tam usuario

        echo
        echo -e "${CYAN}Detalle encontrado (${nombre}):${NC}"
        echo -e "${CYAN}------------------------------------------------------------${NC}"
        ...
        while IFS= read -r -d '' dir; do
            ...
            tam=$(du -sb -- "$dir" 2>/dev/null | awk '{print $1}')
            [[ "$tam" =~ ^[0-9]+$ ]] || tam=0
            usuario=$(basename "$(dirname "$dir")")
            printf " %-24s %-12s %s\n" "$usuario" "$(_nc_human_bytes "$tam")" "$dir"
        done < <(find "$DATA_DIR_LIMPIEZA" -mindepth 2 -maxdepth 2 \
                     -type d -name "$nombre" -print0 2>/dev/null | sort -z)

        if [[ "$encontrados" -eq 0 ]]; then
            echo -e " ${YELLOW}No se encontraron carpetas ${nombre}.${NC}"
        fi
        echo
    }

    # Comprueba si un comando OCC está realmente disponible.
    # Se usa `occ help` porque `occ list --raw` incluye también la descripción
    # y no se puede comparar con grep -x de forma fiable.
    _nc_occ_disponible(){
        local comando="$1"
        sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" help "$comando" \
            --no-ansi --no-interaction >/dev/null 2>&1
    }

    # Los comandos trashbin:* y versions:* pertenecen a apps de Nextcloud.
    # En modo mantenimiento las apps no se cargan, por lo que temporalmente
    # se desactiva mantenimiento para ejecutar la limpieza.
    _nc_preparar_occ_limpieza(){
        NC_RESTAURAR_MANTENIMIENTO=0

        local maint
        maint=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" status --output=json 2>/dev/null \
            | grep -oP '"maintenance"\s*:\s*\K(true|false)' | head -n1)

        if [[ "$maint" == "true" ]]; then
            echo -e "${YELLOW}⚠ Nextcloud está en modo mantenimiento.${NC}"
            echo -e "${CYAN}Desactivándolo temporalmente para cargar files_trashbin/files_versions...${NC}"
            sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" maintenance:mode --off >/dev/null || return 1
            NC_RESTAURAR_MANTENIMIENTO=1
        fi

        return 0
    }

    _nc_restaurar_mantenimiento(){
        if [[ "${NC_RESTAURAR_MANTENIMIENTO:-0}" -eq 1 ]]; then
            echo -e "${CYAN}Restaurando modo mantenimiento...${NC}"
            sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" maintenance:mode --on >/dev/null || true
            NC_RESTAURAR_MANTENIMIENTO=0
        fi
    }

    # Limpieza oficial de papelera por OCC.
    _nc_limpiar_papelera(){
        _nc_preparar_occ_limpieza || {
            err "No fue posible preparar Nextcloud para la limpieza."
            return 1
        }

        if ! _nc_occ_disponible 'trashbin:cleanup'; then
            err "trashbin:cleanup no está disponible. Comprueba que files_trashbin esté habilitada."
            _nc_restaurar_mantenimiento
            return 1
        fi

        echo -e "${CYAN}Limpiando papelera de todos los usuarios mediante OCC...${NC}"
        sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" trashbin:cleanup --all-users --no-interaction
        local estado=$?
        _nc_restaurar_mantenimiento
        return $estado
    }

    # Limpieza oficial de versiones por OCC.
    _nc_limpiar_versiones(){
        _nc_preparar_occ_limpieza || {
            err "No fue posible preparar Nextcloud para la limpieza."
            return 1
        }

        if ! _nc_occ_disponible 'versions:cleanup'; then
            err "versions:cleanup no está disponible. Comprueba que files_versions esté habilitada."
            _nc_restaurar_mantenimiento
            return 1
        fi

        echo -e "${CYAN}Limpiando versiones de todos los usuarios mediante OCC...${NC}"
        sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" versions:cleanup --no-interaction
        local estado=$?
        _nc_restaurar_mantenimiento
        return $estado
    }

    # Elimina solamente el contenido temporal dentro de cada uploads/.
    # La carpeta uploads se conserva.
    _nc_limpiar_uploads(){
        local dir encontrados=0

        echo -e "${CYAN}Eliminando cargas temporales de uploads...${NC}"

        while IFS= read -r -d '' dir; do
            encontrados=1
            find "$dir" -mindepth 1 -maxdepth 1 -exec rm -rf -- {} + 2>/dev/null
        done < <(find "$DATA_DIR_LIMPIEZA" -mindepth 2 -maxdepth 2 \
                     -type d -name uploads -print0 2>/dev/null)

        if [[ "$encontrados" -eq 0 ]]; then
            warn "No se encontraron carpetas uploads."
            return 0
        fi

        ok "✔ Cargas temporales eliminadas."
    }

    while true; do
        clear

        # Detectar la ruta real configurada en Nextcloud.
        DATA_DIR_LIMPIEZA=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" \
            config:system:get datadirectory 2>/dev/null)
        [[ -z "$DATA_DIR_LIMPIEZA" ]] && DATA_DIR_LIMPIEZA="$NEXTCLOUD_DATA"

        if [[ ! -d "$DATA_DIR_LIMPIEZA" ]]; then
            err "No existe el directorio de datos: $DATA_DIR_LIMPIEZA"
            pausa
            return
        fi

        echo -e "${CYAN}${BOLD}=== LIMPIEZA ALMACENAMIENTO NEXTCLOUD ===${NC}"
        echo
        echo -e "${CYAN}Data directory:${NC} $DATA_DIR_LIMPIEZA"
        echo -e "${YELLOW}Escaneando almacenamiento...${NC}"
        echo

        SIZE_TRASH=$(_nc_size_categoria files_trashbin)
        SIZE_VERSIONS=$(_nc_size_categoria files_versions)
        SIZE_UPLOADS=$(_nc_size_categoria uploads)
        SIZE_TOTAL=$((SIZE_TRASH + SIZE_VERSIONS + SIZE_UPLOADS))

        echo -e " ${YELLOW}1)${NC} Papelera       ${CYAN}files_trashbin${NC}  $(_nc_human_bytes "$SIZE_TRASH")"
        echo -e " ${YELLOW}2)${NC} Versiones      ${CYAN}files_versions${NC}  $(_nc_human_bytes "$SIZE_VERSIONS")"
        echo -e " ${YELLOW}3)${NC} Subidas temp.  ${CYAN}uploads${NC}         $(_nc_human_bytes "$SIZE_UPLOADS")"
        echo -e " ${YELLOW}4)${NC} Ver detalle por usuario/carpeta"
        echo -e " ${RED}5)${NC} Limpiar TODO"
        echo
        echo -e " ${GREEN}Espacio potencialmente recuperable:${NC} $(_nc_human_bytes "$SIZE_TOTAL")"
        echo
        echo -e " ${CYAN}0) Volver${NC}"
        echo
        ...
        case "$op_limpieza" in
            1)
                echo
                _nc_detalle_categoria files_trashbin
                [[ "$SIZE_TRASH" -eq 0 ]] && { pausa; continue; }
                confirmar "¿Vaciar la papelera de TODOS los usuarios?" || continue

                ANTES=$(_nc_size_categoria files_trashbin)
                if _nc_limpiar_papelera; then
                    DESPUES=$(_nc_size_categoria files_trashbin)
                    LIBERADO=$((ANTES - DESPUES)); ((LIBERADO < 0)) && LIBERADO=0
                    echo
                    ok "✔ Papelera limpiada. Espacio recuperado: $(_nc_human_bytes "$LIBERADO")"
                else
                    err "No fue posible completar la limpieza de la papelera."
                fi
                pausa
            ;;
            ...
            2)
                echo
                _nc_detalle_categoria files_versions
                [[ "$SIZE_VERSIONS" -eq 0 ]] && { pausa; continue; }
                confirmar "¿Eliminar TODAS las versiones de archivos de todos los usuarios?" || continue

                ANTES=$(_nc_size_categoria files_versions)
                if _nc_limpiar_versiones; then
                    DESPUES=$(_nc_size_categoria files_versions)
                    LIBERADO=$((ANTES - DESPUES)); ((LIBERADO < 0)) && LIBERADO=0
                    echo
                    ok "✔ Versiones limpiadas. Espacio recuperado: $(_nc_human_bytes "$LIBERADO")"
                else
                    err "No fue posible completar la limpieza de versiones."
                fi
                pausa
            ;;
            ...
            3)
                echo
                _nc_detalle_categoria uploads
                [[ "$SIZE_UPLOADS" -eq 0 ]] && { pausa; continue; }
                echo -e "${YELLOW}ADVERTENCIA:${NC} uploads contiene cargas temporales/incompletas."
                echo -e "${YELLOW}No ejecutes esta limpieza mientras haya usuarios subiendo archivos.${NC}"
                confirmar "¿Eliminar el contenido de TODAS las carpetas uploads?" || continue

                ANTES=$(_nc_size_categoria uploads)
                _nc_limpiar_uploads
                DESPUES=$(_nc_size_categoria uploads)
                LIBERADO=$((ANTES - DESPUES)); ((LIBERADO < 0)) && LIBERADO=0
                echo
                ok "✔ Uploads limpiado. Espacio recuperado: $(_nc_human_bytes "$LIBERADO")"
                pausa
            ;;
            ...
            5)
                [[ "$SIZE_TOTAL" -eq 0 ]] && {
                    ok "No hay espacio que limpiar."
                    pausa
                    continue
                }

                echo
                echo -e "${RED}⚠ Se vaciará la papelera, se eliminarán las versiones y se borrarán uploads temporales.${NC}"
                echo -e "${YELLOW}No debe haber cargas de archivos activas mientras se limpia uploads.${NC}"
                echo -e "${CYAN}Espacio detectado:${NC} $(_nc_human_bytes "$SIZE_TOTAL")"
                echo
                confirmar "¿CONFIRMAR LIMPIEZA TOTAL?" || continue

                ANTES_TOTAL=$(_nc_size_categoria files_trashbin)
                ANTES_TOTAL=$((ANTES_TOTAL + $(_nc_size_categoria files_versions)))
                ANTES_TOTAL=$((ANTES_TOTAL + $(_nc_size_categoria uploads)))

                _nc_limpiar_papelera || warn "La papelera no pudo limpiarse completamente."
                echo
                _nc_limpiar_versiones || warn "Las versiones no pudieron limpiarse completamente."
                echo
                _nc_limpiar_uploads || warn "Uploads no pudo limpiarse completamente."

                # Limpia entradas huérfanas del file cache si el comando está disponible.
                if _nc_occ_disponible 'files:cleanup'; then
                    echo
                    echo -e "${CYAN}Revisando entradas huérfanas del file cache...${NC}"
                    sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" files:cleanup --no-interaction || true
                fi

                DESPUES_TOTAL=$(_nc_size_categoria files_trashbin)
                DESPUES_TOTAL=$((DESPUES_TOTAL + $(_nc_size_categoria files_versions)))
                DESPUES_TOTAL=$((DESPUES_TOTAL + $(_nc_size_categoria uploads)))
                LIBERADO=$((ANTES_TOTAL - DESPUES_TOTAL)); ((LIBERADO < 0)) && LIBERADO=0

                echo
                echo -e "${GREEN}════════════════════════════════════════════${NC}"
                ok "✔ LIMPIEZA TOTAL COMPLETADA"
                echo -e "${GREEN}Espacio recuperado:${NC} $(_nc_human_bytes "$LIBERADO")"
                echo -e "${GREEN}Espacio restante en estas carpetas:${NC} $(_nc_human_bytes "$DESPUES_TOTAL")"
                echo -e "${GREEN}════════════════════════════════════════════${NC}"
                pausa
            ;;
...
# CIERRE FUNCION AVANCE
    ...
    VERSION_ACTUAL=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" status --output=json \
        | grep -oP '"versionstring":\s*"\K[^"]+')

    echo -e "Versión actual: ${YELLOW}$VERSION_ACTUAL${NC}"

    CHECK=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" update:check 2>&1)
    echo "$CHECK"

    NUEVA_VERSION=$(echo "$CHECK" \
        | grep -oP '[0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?' \
        | head -n1)

    if [[ -z "$NUEVA_VERSION" ]]; then
        warn "No se detectó una nueva versión disponible"
        pausa
        break
    fi
    ...
    DB_BACKUP=$(backup_db_nc) || {
        err "Error realizando backup de base de datos"
        pausa
        break
    }
    ...
    # ==========================================
    # EJECUTAR UPDATER OFICIAL
    # ==========================================
    #
    # --no-interaction hace automáticamente:
    #
    # - comprobaciones
    # - mantenimiento ON
    # - backup interno Nextcloud
    # - descarga
    # - extracción
    ...
    echo
    echo -e "${CYAN}Ejecutando actualizador oficial de Nextcloud...${NC}"
    echo

    if ! sudo -E -u "$USER_WEB" \
        php --define apc.enable_cli=1 \
        "$NEXTCLOUD_DIR/updater/updater.phar" \
        --no-interaction
    then
        echo
        err "El actualizador de Nextcloud informó un error"

        echo
        warn "NO se restaurará automáticamente la base de datos."
        warn "El updater puede haber reemplazado ya archivos de Nextcloud."
        warn "Restaurar solamente la BD podría dejar archivos y BD desincronizados."

        echo
        echo -e "${YELLOW}Revise el log del updater antes de continuar.${NC}"

        sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" status 2>/dev/null || true
    ...
    echo
    echo -e "${CYAN}Comprobando estado después de actualizar...${NC}"

    if ! sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" status; then
        err "No fue posible comprobar el estado de Nextcloud"
        pausa
        break
    fi
    ...
    run_live "Ejecutando autoreparación" autoreparacion_nc || {
        warn "La actualización terminó, pero autoreparacion_nc informó un problema"
    }

    # ==========================================
    # ASEGURAR SALIDA DE MANTENIMIENTO
    # ==========================================

    sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" maintenance:mode --off >/dev/null 2>&1 || true

    # ==========================================
    # VERSIÓN FINAL
    # ==========================================

    VERSION_FINAL=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" status --output=json \
        | grep -oP '"versionstring":\s*"\K[^"]+')

    echo
    echo -e "${GREEN}✔ Nextcloud actualizado correctamente${NC}"
    echo -e "Versión anterior : ${YELLOW}$VERSION_ACTUAL${NC}"
    echo -e "Versión instalada: ${GREEN}$VERSION_FINAL${NC}"
...
# ========= Ajustes de Red =========
# ==========================================
# MENU RED + WAKE-ON-LAN
# ==========================================

# ---------- WOL ----------

wol_instalar_ethtool() {
    command -v ethtool >/dev/null 2>&1 || {
        echo "[INFO] Instalando ethtool..."
...
wol_listar_interfaces() {
    echo
    echo -e "${CYAN}${BOLD}Interfaces disponibles:${NC}"
    ip -o link show | awk -F': ' '{print $2}' | grep -Ev '^(lo)$'
    echo
}
...
wol_verificar() {
    ...
    clear
    echo -e "${CYAN}${BOLD}=== VERIFICACIÓN WAKE-ON-LAN ===${NC}"
    echo
    ...
    for iface in $(ip -o link show | awk -F': ' '{print $2}' | grep -Ev '^(lo|docker|veth|br-|vmbr)'); do

        soporta=$(ethtool "$iface" 2>/dev/null | awk -F': ' '/Supports Wake-on/ {print $2}')
        actual=$(ethtool "$iface" 2>/dev/null | awk -F': ' '/Wake-on/ {print $2}' | tail -1)

        if [ -n "$soporta" ] && [ "$soporta" != "d" ]; then
            ok "$iface soporta Wake-on-LAN ($soporta) - Estado: $actual"
            found=1
        else
            warn "$iface no soporta Wake-on-LAN"
        fi
    done
    ...
    if [ "$found" -eq 0 ]; then
        err "No se encontraron interfaces compatibles con WOL."
    fi
...
wol_estado() {
    ...
    read -rp "Interfaz: " iface

    if ! ip link show "$iface" >/dev/null 2>&1; then
        err "Interfaz inválida."
        pausa
        return
    fi
...
wol_activar() {
    ...
    if [ -z "$soporta" ] || [ "$soporta" = "d" ]; then
        err "La interfaz no soporta Wake-on-LAN."
        pausa
        return
    fi

    if ethtool -s "$iface" wol g; then
        ok "Wake-on-LAN activado en $iface"
    else
        err "No se pudo activar Wake-on-LAN."
    fi
...
wol_persistente() {
    ...
    cat > "/etc/systemd/system/wol-${iface}.service" << EOF
[Unit]
Description=Wake-on-LAN ${iface}
...
EOF
    ...
    systemctl enable "wol-${iface}.service" >/dev/null 2>&1

    ok "Persistencia activada para ${iface}"
...
menu_red(){
  ...
  while true; do
    ...
    echo -e " ${YELLOW}4)${NC} Menú Wake-on-LAN (WOL)"
    echo -e " ${CYAN}0)${NC} Volver"
    echo
    ...
    case "$op" in
      ...
      1)
        sudo "$EDITOR_BIN" /etc/network/interfaces
        pausa
        ;;

      2)
        sudo systemctl restart networking \
          && ok "Servicio de red reiniciado." \
          || err "Error al reiniciar la red."
        pausa
        ;;

      3)
        read -rp "Número de puerto a permitir: " p

        sudo iptables -A INPUT -p tcp --dport "$p" -j ACCEPT \
          && ok "Puerto $p permitido." \
          || err "Error al abrir el puerto."
...
montar_sin_formato_nct(){
    ...
    if [[ "$RESP" =~ ^[sS]$ ]]; then
        if grep -q "$UUID" /etc/fstab; then
            ...
            echo "UUID=$UUID $MNT $FSTYPE ${OPTIONS},nofail,x-systemd.device-timeout=10s 0 2" >> /etc/fstab
...
case "$FSTYPE" in
    ext4)
        ...
        echo "UUID=$UUID $MNT ext4 defaults,acl,noatime,nofail,x-systemd.device-timeout=10s 0 2" >> /etc/fstab
        ;;
    *)
        # Montaje de otros sistemas de archivos (NTFS, exFAT, etc.) con uid/gid y nofail
        echo "UUID=$UUID $MNT $FSTYPE defaults,uid=$(id -u $WEB_USER),gid=$(id -g $WEB_USER),umask=002,nofail,x-systemd.device-timeout=10s 0 0" >> /etc/fstab
        ;;
esac

echo "✔ Entrada agregada a /etc/fstab con nofail + timeout seguro de 10s"
    ...
    case $OPC in
      ...
      2)
        ...
        if grep -q "$UUID" /etc/fstab; then
          ...
          echo "UUID=$UUID $MNT $FSTYPE ${OPTS},nofail,x-systemd.device-timeout=10s 0 2" >> /etc/fstab
...
while true; do
  clear
echo -e "${BOLD}${CYAN}=== MENÚ NEXTCLOUD - Turnkey ${YELLOW}v24.3.5 ${CYAN}===${NC}"
echo -e " ${YELLOW}↑"
...
echo -e " ${YELLOW}*"
echo -e " ${YELLOW}5)${NC} Red (Firewall/IP Manual/WOL)"
...
# 🔥 LIMPIEZA FINAL (CLAVE PARA QUE NO QUEDE EL MENÚ)
clear
printf '\033c'