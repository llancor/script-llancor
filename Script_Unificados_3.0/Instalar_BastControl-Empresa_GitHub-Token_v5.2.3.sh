...
migrate_legacy_env(){
  ...
  for legacy_env in \
    "/opt/bast-control-empresa/bastcontrol/.env" \
    "/opt/bast-control-empresa/bastcontrol/.env" \
    "/opt/bast-control-empresa/BastControl/.env"; do
...
download_project(){
  ...
  temp_repo="$(mktemp -d /tmp/bastcontrol-empresa.XXXXXX)" || return 1
...
"
  ...
  # Copia únicamente BastControl-Empresa directamente en la ruta final.
...
while true; do
  ...
  printf "  ${Y}10)${Y} Actualizar BastControl Empresa GitHub-Token${N}\n"
