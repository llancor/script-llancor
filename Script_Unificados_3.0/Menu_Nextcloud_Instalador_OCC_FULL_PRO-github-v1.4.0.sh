...
# ========= MENÚ USUARIOS NEXTCLOUD =========
menu_usuarios_nextcloud(){
  while true; do
    clear
    echo -e "${CYAN}${BOLD}=== USUARIOS NEXTCLOUD ===${NC}"
    echo -e " ${YELLOW}1)${NC} Listar usuarios"
    echo -e " ${YELLOW}2)${NC} Crear usuario"
    echo -e " ${YELLOW}3)${NC} Eliminar usuario"
    echo -e " ${YELLOW}4)${NC} Resetear contraseña (seguro)"
    echo -e " ${YELLOW}5)${NC} Cambiar contraseña (automático)"
    echo -e " ${YELLOW}6)${NC} Hacer admin a usuario"
    echo -e " ${YELLOW}7)${NC} Ver info de usuario"
    echo -e " ${CYAN}0) Volver${NC}"
    read -rp "> " op
...
# ===== LISTAR USUARIOS =====
nc_listar_usuarios(){
  echo -e "${CYAN}Listando usuarios...${NC}"
  sudo -u $NC_USER php $NC_PATH/occ user:list
  pausa
}

# =========================================================
# CREAR USUARIO REUTILIZANDO DATA EXISTENTE
# =========================================================

nc_crear_usuario(){

    echo
    read -rp "Nuevo usuario: " user
    ...
    TEMP_DIR="${DATA_DIR}/.${user}_backup_$(date +%s)"

    # =====================================================
    # VERIFICAR SI YA EXISTE EN NEXTCLOUD
    # =====================================================

    if sudo -u "$NC_USER" php "$NC_PATH/occ" user:info "$user" >/dev/null 2>&1; then

        err "El usuario ya existe en Nextcloud"
        pausa
        return
    ...
    if [ -d "$USER_DIR" ]; then
        ...
        echo
        echo -e "${YELLOW}⚠ Carpeta existente detectada${RESET}"
        echo -e "${CYAN}$USER_DIR${RESET}"
        echo
        ...
        if [[ "$RESP" =~ ^[sS]$ ]]; then
            ...
            echo
            echo -e "${CYAN}Moviendo carpeta temporalmente...${RESET}"

            mv "$USER_DIR" "$TEMP_DIR"

            if [ $? -ne 0 ]; then
                err "No se pudo mover la carpeta"
                pausa
                return
            fi
            ...
            echo -e "${GREEN}✔ Carpeta respaldada${RESET}"
    ...
    sudo -u "$NC_USER" php "$NC_PATH/occ" user:add "$user"
    ...
    if [ $? -ne 0 ]; then
        ...
        # Restaurar si falló
        if [ "$RESTORE" = "1" ] && [ -d "$TEMP_DIR" ]; then
            mv "$TEMP_DIR" "$USER_DIR"
        fi
    ...
    # =====================================================
    # RESTAURAR SOLO ARCHIVOS REALES
    # =====================================================

    if [ "$RESTORE" = "1" ]; then

        echo
        echo -e "${CYAN}Restaurando archivos...${RESET}"

        mkdir -p "$USER_DIR/files"

        # =================================================
        # MOVER SOLO ARCHIVOS ÚTILES
        # =================================================
        ...
        rm -rf "$USER_DIR/files/cache" 2>/dev/null
        rm -rf "$USER_DIR/files/uploads" 2>/dev/null
        rm -rf "$USER_DIR/files/files_trashbin" 2>/dev/null
        rm -rf "$USER_DIR/files/files_versions" 2>/dev/null
    ...
    sudo -u "$NC_USER" php "$NC_PATH/occ" files:scan --path="$user/files"
...
nc_eliminar_usuario(){
    ...
    if [ -z "$USERS" ]; then
        warn "No hay usuarios disponibles"
        pausa
        return
    fi
    ...
    echo
    echo -e "${YELLOW}Usuario seleccionado:${RESET} $user"
    echo

    printf "${CYAN}¿Eliminar también los archivos DATA del usuario? (s/n): ${RESET}"
    read DEL_DATA
    ...
    echo
    echo -e "${CYAN}Eliminando usuario...${RESET}"

    sudo -u "$NC_USER" php "$NC_PATH/occ" user:delete "$user"
    ...
    if [[ "$DEL_DATA" =~ ^[sS]$ ]]; then
        ...
        echo
        echo -e "${CYAN}Eliminando archivos del usuario...${RESET}"
        ...
        echo
        echo -e "${YELLOW}DATA conservada:${RESET} $USER_DIR"
...
nc_resetear_password(){
  ...
  echo -e "${CYAN}Modo seguro (pedirá contraseña 2 veces)...${NC}"

  sudo -u $NC_USER php $NC_PATH/occ user:resetpassword "$user"

  if [ $? -eq 0 ]; then
    ok "Contraseña actualizada correctamente"
  else
    err "Error al cambiar contraseña"
  fi

  pausa
}

# ===== CAMBIAR PASSWORD AUTOMÁTICO =====
nc_cambiar_password(){
  read -rp "Usuario: " user
  ...
  if [ -z "$user" ] || [ -z "$pass" ]; then
    warn "Datos inválidos"
    pausa
    return
  fi

  echo -e "${CYAN}Aplicando contraseña automáticamente...${NC}"

  if sudo -u $NC_USER OC_PASS="$pass" php $NC_PATH/occ user:resetpassword --password-from-env "$user"; then
...
nc_hacer_admin(){
  ...
  sudo -u $NC_USER php $NC_PATH/occ group:adduser admin "$user"

  if [ $? -eq 0 ]; then
    ok "Usuario ahora es administrador"
  else
    err "Error asignando admin"
  fi
...
nc_info_usuario(){
  ...
  sudo -u $NC_USER php $NC_PATH/occ user:info "$user"
  pausa
}
...
check_cluster_dependencies(){
    ...
    echo
    echo -e "${CYAN}Verificando dependencias Cluster SSH...${RESET}"
...
run_command_cluster(){
    ...
    echo
    echo -e "${CYAN}=== EJECUTAR COMANDO EN CLUSTER ===${RESET}"

    read -rp "Usuario SSH: " USER
    read -rsp "Password SSH: " PASS
    echo

    read -rp "Comando: " CMD
    read -rp "Ejecutar como root? (s/n): " ROOT
    read -rp "IP o rango: " RANGO

    mapfile -t HOSTS < <(parse_rango "$RANGO")

    TOTAL=${#HOSTS[@]}
    COMPLETADOS=0

    echo
    echo -e "${YELLOW}Iniciando ejecución en $TOTAL hosts...${RESET}"
    ...
    for HOST in "${HOSTS[@]}"; do
        ...
        (
            ...
            if check_host "$HOST"; then
                ...
                if [[ "$ROOT" =~ ^[sS]$ ]]; then
                    REMOTE_CMD="echo '$PASS' | sudo -S bash -c '$CMD'"
                else
                    REMOTE_CMD="$CMD"
                fi
                ...
                if [ $? -eq 0 ]; then
                    echo "[OK] Comando ejecutado correctamente" >> "$LOG"
                else
                    echo "[ERROR] Falló ejecución" >> "$LOG"
                fi
    ...
    echo
    echo -e "${GREEN}✔ Ejecución completada${RESET}"
...
copy_file_cluster(){
    ...
    echo
    echo -e "${CYAN}=== COPIAR ARCHIVO EN CLUSTER ===${RESET}"

    read -rp "Usuario SSH: " USER
    read -rsp "Password SSH: " PASS
    echo

    read -rp "Archivo local: " FILE
    read -rp "Ruta remota destino: " DEST
    read -rp "IP o rango: " RANGO
    ...
    echo
    echo -e "${YELLOW}Iniciando copia en $TOTAL hosts...${RESET}"
    ...
    for HOST in "${HOSTS[@]}"; do
        ...
        (
            ...
            if check_host "$HOST"; then
                ...
                if [ $? -eq 0 ]; then
                    echo "[OK] Archivo copiado" >> "$LOG"
                else
                    echo "[ERROR] Error copiando archivo" >> "$LOG"
                fi
    ...
    echo
    echo -e "${GREEN}✔ Copia completada${RESET}"
...
run_script_cluster(){
    ...
    echo
    echo -e "${CYAN}=== EJECUTAR SCRIPT REMOTO ===${RESET}"

    read -rp "Usuario SSH: " USER
    read -rsp "Password SSH: " PASS
    echo

    read -rp "Script local (.sh): " SCRIPT
    read -rp "IP o rango: " RANGO

    if [ ! -f "$SCRIPT" ]; then
        echo -e "${RED}Script no encontrado${RESET}"
        sleep 2
        return
    fi
        ...
        (
            ...
            if check_host "$HOST"; then
                ...
                sshpass -p "$PASS" ssh \
                    ...
                    -o ConnectTimeout=5 \
                    "$USER@$HOST" \
                    "chmod +x /tmp/script.sh && bash /tmp/script.sh" >> "$LOG" 2>&1

                if [ $? -eq 0 ]; then
                    echo "[OK] Script ejecutado correctamente" >> "$LOG"
                else
                    echo "[ERROR] Error ejecutando script" >> "$LOG"
                fi
    ...
    echo
    echo -e "${GREEN}✔ Ejecución finalizada${RESET}"
...
cluster_menu(){
    ...
    while true; do
        ...
        echo -e "${YELLOW}=======================================${RESET}"
        echo -e "${YELLOW}           CLUSTER SSH PRO             ${RESET}"
        echo -e "${YELLOW}=======================================${RESET}"

        echo "1) Ejecutar comando"
        echo "2) Copiar archivo"
        echo "3) Ejecutar script remoto"
        echo "0) Volver"
...
menu_wol(){
  while true; do
    clear
    echo -e "${BOLD}${CYAN}=== WAKE-ON-LAN / WAKE-ON-WLA ===${NC}"
    echo -e " ${YELLOW}1)${NC} Enviar Wake-on-LAN (LAN local)"
    echo -e " ${YELLOW}2)${NC} Enviar Wake-on-LAN por Internet (WLA)"
    ...
    case "$opc" in
      1)
        echo -e "\n${CYAN}=== Equipos LAN ===${NC}"
        i=1
        for host in "${HOSTS_LAN[@]}"; do
          name="${host%%:*}"
          mac="${host#*:}"
          echo -e " ${YELLOW}$i)${NC} ${GREEN}$name${NC} (${BLUE}$mac${NC})"
          ((i++))
        done
        echo -e " ${YELLOW}0)${NC} ${GREEN}Encender TODOS${NC}\n"
        read -p "Elige una opción: " opt
        
        if [[ "$opt" == "0" ]]; then
          echo -e "${CYAN}Encendiendo TODOS los equipos LAN...${NC}"
          for host in "${HOSTS_LAN[@]}"; do
            name="${host%%:*}"
            mac="${host#*:}"
            wakeonlan -i "$BCAST" "$mac"
            echo -e "${GREEN} -> Enviado WOL a $name ($mac)${NC}"
          done
        elif [[ "$opt" =~ ^[0-9]+$ ]] && (( opt > 0 && opt <= ${#HOSTS_LAN[@]} )); then
          host="${HOSTS_LAN[$((opt-1))]}"
          name="${host%%:*}"
          mac="${host#*:}"
          echo -e "${CYAN}Enviando paquete WOL a $name ($mac)...${NC}"
          wakeonlan -i "$BCAST" "$mac"
          echo -e "${GREEN} -> Enviado WOL a $name${NC}"
        else
          err "Opción inválida"
        fi
        pausa
        ;;
      2)
        echo -e "\n${CYAN}=== Equipos WLA (Internet) ===${NC}"
        i=1
        for host in "${HOSTS_WLA[@]}"; do
          IFS=':' read -r name mac dominio port <<< "$host"
          echo -e " ${YELLOW}$i)${NC} ${GREEN}$name${NC} (${BLUE}$mac${NC}) -> ${MAGENTA}$dominio:$port${NC}"
          ((i++))
        done
        echo -e " ${YELLOW}0)${NC} ${GREEN}Encender TODOS${NC}\n"
        read -p "Elige una opción: " opt
        ...
        if [[ "$opt" == "0" ]]; then
          echo -e "${CYAN}Encendiendo TODOS los equipos WLA...${NC}"
          for host in "${HOSTS_WLA[@]}"; do
            IFS=':' read -r name mac dominio port <<< "$host"
            wakeonlan -i "$dominio" -p "$port" "$mac"
            echo -e "${GREEN} -> Enviado WLA a $name ($dominio:$port)${NC}"
          done
        elif [[ "$opt" =~ ^[0-9]+$ ]] && (( opt > 0 && opt <= ${#HOSTS_WLA[@]} )); then
          host="${HOSTS_WLA[$((opt-1))]}"
          IFS=':' read -r name mac dominio port <<< "$host"
          echo -e "${CYAN}Enviando paquete WLA a $name ($mac)...${NC}"
          wakeonlan -i "$dominio" -p "$port" "$mac"
          echo -e "${GREEN} -> Enviado WLA a $name${NC}"
        else
          err "Opción inválida"
        fi
        pausa
...
menu_nextcloud_occ_pro(){
  while true; do
    clear
echo -e "${BOLD}${CYAN}=== MENÚ NEXTCLOUD - Turnkey ${YELLOW}v24.0 ${CYAN}===${NC}"
echo -e " ${YELLOW}↑"
...
echo
echo -e "${CYAN}====================================================${RESET}"
echo -e "${CYAN}       NEXTCLOUD PRO v7.8.0 PRO VIP GitHub V1.4.0   ${RESET}"
echo -e "${CYAN}====================================================${RESET}"
echo
...
echo -e "${YELLOW}8)${RESET}  ${GREEN}Descargar Script desde GITHUB${RESET}"
echo
