...
install_wordpress() {
    ...
    echo -e "${CYAN}Descargando WordPress...${RESET}"

    wget -O /tmp/wp_latest.zip https://wordpress.org/latest.zip

    if [ $? -ne 0 ]; then
        echo -e "${RED}✘ Error descargando WordPress${RESET}"
        return 1
    fi
    ...
    if [ -d "$wp_path" ]; then
        echo -e "${YELLOW}La carpeta ya existe: $wp_path${RESET}"

        read -rp "¿Desea continuar y sobrescribir? (s/n): " resp
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}✘ Error creando base de datos${RESET}"
        return 1
    fi


    # =====================================
    # Crear wp-config con WP-CLI
    # =====================================

    echo -e "${CYAN}Creando wp-config.php...${RESET}"


    sudo -u "$USER_WEB" wp config create \
        --path="$wp_path" \
        --dbname="$WP_DBNAME" \
        --dbuser="$WP_DBUSER" \
        --dbpass="$WP_DBPASS" \
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}✘ Error creando wp-config.php${RESET}"
        return 1
    fi
    ...
    echo
    echo -e "${CYAN}Instalando WordPress...${RESET}"


    read -rp "Título del sitio [Mi WordPress]: " WP_TITLE
    WP_TITLE=${WP_TITLE:-Mi WordPress}


    read -rp "Usuario administrador [admin]: " WP_ADMIN
    WP_ADMIN=${WP_ADMIN:-admin}


    read -rsp "Contraseña administrador: " WP_ADMIN_PASS
    echo


    while true; do

    read -rp "Correo administrador: " WP_ADMIN_EMAIL

    if [[ "$WP_ADMIN_EMAIL" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$ ]]; then
        break
    else
        echo -e "${RED}Correo inválido. Ejemplo: usuario@gmail.com${RESET}"
    fi

done


sudo -u "$USER_WEB" wp core install \
    --path="$wp_path" \
    --url="https://$WP_DOMAIN" \
    --title="$WP_TITLE" \
    --admin_user="$WP_ADMIN" \
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}✘ Error instalando WordPress${RESET}"
        return 1
    fi
    ...
    echo -e "${CYAN}Configurando enlaces permanentes...${RESET}"


    sudo -u "$USER_WEB" wp rewrite structure \
        '/%postname%/' \
        --path="$wp_path"


    sudo -u "$USER_WEB" wp rewrite flush \
        --path="$wp_path"
...
configurar_php_upload() {

    PHPINI=$(find /etc/php -type f -path "*/apache2/php.ini" | sort -V | tail -n1)

    if [ ! -f "$PHPINI" ]; then
        echo -e "${RED}No se encontró el php.ini de Apache.${RESET}"
        return
    fi

    echo -e "${CYAN}Configurando límites de PHP...${RESET}"

    cp "$PHPINI" "${PHPINI}.bak.$(date +%Y%m%d_%H%M%S)"

    configurar_php() {
        local PARAM="$1"
        local VALOR="$2"

        if grep -qE "^[[:space:];]*${PARAM}[[:space:]]*=" "$PHPINI"; then
            sed -i -E "s|^[[:space:];]*${PARAM}[[:space:]]*=.*|${PARAM} = ${VALOR}|g" "$PHPINI"
        else
            echo "${PARAM} = ${VALOR}" >> "$PHPINI"
        fi
    }

    configurar_php upload_max_filesize 2048M
    configurar_php post_max_size 2048M
    configurar_php memory_limit 512M
    configurar_php max_execution_time 600
    configurar_php max_input_time 600

    systemctl restart apache2

    echo
    echo -e "${GREEN}✓ PHP configurado correctamente.${RESET}"
    echo -e "${WHITE}Archivo:${RESET} $PHPINI"
    echo

    php -c "$PHPINI" -i | grep -E "upload_max_filesize|post_max_size|memory_limit|max_execution_time|max_input_time"
}
instalar_wpcli() {

    if command -v wp >/dev/null 2>&1; then
        echo -e "${GREEN}✔ WP-CLI ya está instalado.${RESET}"
        return
    fi

    echo -e "${CYAN}Instalando WP-CLI...${RESET}"
    ...
    if command -v wp >/dev/null 2>&1; then
        echo -e "${GREEN}✔ WP-CLI instalado correctamente.${RESET}"
    else
        echo -e "${RED}✘ Error al instalar WP-CLI.${RESET}"
        return 1
    fi
}
# =========================================
...
while true; do
    ...
    echo -e "${WHITE}===============================================${RESET}"
    echo -e "${CYAN} Instalador WordPress Automático v11.2 ${RESET}"
    echo -e "${WHITE}===============================================${RESET}"
	...
	echo -e "${YELLOW}6)${RESET} Configurar límites de subida PHP"
