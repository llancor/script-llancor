...
SCRIPT="/root/Gestion_UPS_APC-SRV3KI_Cliente_LAN_v4.9-corregido.sh"
...
############################################################
# ESTADOS NUT / MONITOR APC LAN / APAGADO
############################################################
...
nut_monitor_activo() {
    ...
    pgrep -x upsmon >/dev/null 2>&1
}

nut_monitor_estado_color() {
    if systemctl is-active --quiet nut-monitor.service 2>/dev/null; then
        echo -e "${VERDE}ACTIVO${RESET}"
    elif pgrep -x upsmon >/dev/null 2>&1; then
        echo -e "${AMARILLO}ACTIVO (proceso upsmon)${RESET}"
    else
        echo -e "${ROJO}DETENIDO${RESET}"
    fi
}

ups_apc_monitor_activo() {
    systemctl is-active --quiet "$NOMBRE_SERVICIO" 2>/dev/null
}

ups_apc_monitor_habilitado() {
    systemctl is-enabled --quiet "$NOMBRE_SERVICIO" 2>/dev/null
}

mostrar_estado_proteccion_resumido() {
    cargar_configuracion

    echo
    echo -e "${NEGRITA}MONITOREO Y PROTECCIÓN${RESET}"
    linea

    printf "%-31s " "NUT Monitor (upsmon):"
    nut_monitor_estado_color

    if ups_apc_monitor_activo; then
        echo -e "UPS APC Monitor LAN            : ${VERDE}ACTIVO${RESET}"
    else
        echo -e "UPS APC Monitor LAN            : ${ROJO}DETENIDO${RESET}"
    fi

    if ups_apc_monitor_habilitado; then
        echo -e "Inicio Monitor APC LAN         : ${VERDE}HABILITADO${RESET}"
    else
        echo -e "Inicio Monitor APC LAN         : ${AMARILLO}DESHABILITADO${RESET}"
    fi

    if [[ "${MONITOR_APAGADO^^}" == "ON" ]]; then
        echo -e "Apagado automático             : ${VERDE}ACTIVADO${RESET}"
    else
        echo -e "Apagado automático             : ${AMARILLO}BLOQUEADO (OFF)${RESET}"
    fi

    printf "%-31s %s segundos\n" "Espera tras corte:" "$WAIT_POWER_FAIL"
...
EOF
    ...
    cp -a /etc/nut/upsmon.conf /etc/nut/upsmon.conf.bak

    # Limpiar entradas anteriores para evitar duplicados o valores
    # inconsistentes que hagan fallar upsmon.
    sed -i -E '/^[[:space:]]*MONITOR[[:space:]]+/d' /etc/nut/upsmon.conf
    sed -i -E '/^[[:space:]]*SHUTDOWNCMD[[:space:]]+/d' /etc/nut/upsmon.conf
    sed -i -E '/^[[:space:]]*MINSUPPLIES[[:space:]]+/d' /etc/nut/upsmon.conf
...
EOF
    ...
    ########################################################
    # VALIDAR CONFIGURACION ANTES DE INICIAR nut-monitor
    ########################################################

    MONITOR_ESPERADO="${UPSNAME}@${SERVER_IP}:${PUERTO}"

    if ! grep -Eq "^[[:space:]]*MONITOR[[:space:]]+${UPSNAME}@${SERVER_IP}:${PUERTO}[[:space:]]+1[[:space:]]+${USUARIO}[[:space:]]+[^[:space:]]+[[:space:]]+secondary[[:space:]]*$" /etc/nut/upsmon.conf
    then
        mensaje_error "No se pudo crear una línea MONITOR válida en /etc/nut/upsmon.conf"
        echo
        echo "Se esperaba una entrada para:"
        echo "  $MONITOR_ESPERADO"
        echo
        echo "Configuración actual:"
        grep -nE '^[[:space:]]*(MONITOR|MINSUPPLIES|SHUTDOWNCMD)' /etc/nut/upsmon.conf 2>/dev/null
        pausa
        return
    fi

    if ! grep -Eq '^[[:space:]]*MINSUPPLIES[[:space:]]+1[[:space:]]*$' /etc/nut/upsmon.conf
    then
        mensaje_error "MINSUPPLIES no quedó configurado en 1."
        pausa
        return
    fi

    if ! upsc "${UPSNAME}@${SERVER_IP}:${PUERTO}" >/dev/null 2>&1
    then
        mensaje_error "La configuración MONITOR es válida, pero no se puede consultar la UPS remota."
        echo
        echo "Pruebe manualmente:"
        echo "  upsc ${UPSNAME}@${SERVER_IP}:${PUERTO}"
        echo
        echo "Revise IP, puerto 3493, firewall y servidor NUT."
        pausa
        return
    fi
    ...
    systemctl reset-failed nut-monitor.service >/dev/null 2>&1
    systemctl enable nut-monitor.service >/dev/null 2>&1

    if ! systemctl restart nut-monitor.service
    then
        mensaje_error "nut-monitor no pudo iniciarse."

        echo
        echo "Diagnóstico:"
        systemctl status nut-monitor.service --no-pager -l
        echo
        journalctl -u nut-monitor.service -n 30 --no-pager
        pausa
        return
    fi
    ...
    if systemctl is-active --quiet nut-monitor.service
    then

        mensaje_ok "Cliente NUT iniciado correctamente."

    else

        mensaje_error "nut-monitor no quedó activo."
    ...
    echo
    echo "Configuración crítica de upsmon:"
    grep -nE '^[[:space:]]*(MONITOR|MINSUPPLIES|SHUTDOWNCMD)' /etc/nut/upsmon.conf 2>/dev/null
...
configurar_shutdown() {
    ...
    echo
    echo "Estado actual del apagado automático: $MONITOR_APAGADO"
    read -rp "¿Activar apagado automático? (ON/OFF) [$MONITOR_APAGADO]: " TMP
    ...
    then
        ...
        then
            ...
            echo -e "${AMARILLO}⚠ Valor inválido. Se conservará MONITOR_APAGADO=$MONITOR_APAGADO${RESET}"
        fi
    fi
...
############################################################
# VERIFICAR APAGADO AUTOMATICO
############################################################

verificar_apagado_automatico() {

    cargar_configuracion

    header
    titulo "VERIFICACION APAGADO AUTOMATICO UPS"
    echo

    local ERRORES=0
    local AVISOS=0
    local MODO_NUT
    local SERVICIO_NUT_LOCAL
    local UPS_OBJETIVO_LOCAL
    local ESTADO_UPS="N/D"
    local BATERIA_UPS="N/D"
    local AUTONOMIA_UPS="N/D"

    echo -e "${NEGRITA}Configuración${RESET}"
    linea

    if [[ "${MONITOR_APAGADO^^}" == "ON" ]]; then
        echo -e "${OK} Apagado automático       : ${VERDE}ACTIVADO${RESET}"
    elif [[ "${MONITOR_APAGADO^^}" == "OFF" ]]; then
        echo -e "${WARNING} Apagado automático       : ${AMARILLO}BLOQUEADO (OFF)${RESET}"
        ((AVISOS++))
    else
        echo -e "${ERROR} Apagado automático       : ${ROJO}VALOR INVÁLIDO${RESET} (${MONITOR_APAGADO:-NO DEFINIDO})"
        ((ERRORES++))
    fi

    if [[ "$WAIT_POWER_FAIL" =~ ^[0-9]+$ ]]; then
        echo -e "${OK} Espera después del corte : ${WAIT_POWER_FAIL} segundos"
    else
        echo -e "${ERROR} Espera después del corte : valor inválido (${WAIT_POWER_FAIL:-vacío})"
        ((ERRORES++))
    fi

    if [[ "$BATTERY_LIMIT" =~ ^[0-9]+$ ]] && (( BATTERY_LIMIT >= 0 && BATTERY_LIMIT <= 100 )); then
        echo -e "${OK} Batería mínima           : ${BATTERY_LIMIT} %"
    else
        echo -e "${ERROR} Batería mínima           : valor inválido (${BATTERY_LIMIT:-vacío})"
        ((ERRORES++))
    fi

    if [[ "$RUNTIME_LIMIT" =~ ^[0-9]+$ ]]; then
        echo -e "${OK} Autonomía mínima         : ${RUNTIME_LIMIT} segundos"
    else
        echo -e "${ERROR} Autonomía mínima         : valor inválido (${RUNTIME_LIMIT:-vacío})"
        ((ERRORES++))
    fi

    if [[ "$MONITOR_INTERVAL" =~ ^[0-9]+$ ]] && (( MONITOR_INTERVAL > 0 )); then
        echo -e "${OK} Intervalo monitoreo      : ${MONITOR_INTERVAL} segundos"
    else
        echo -e "${ERROR} Intervalo monitoreo      : valor inválido (${MONITOR_INTERVAL:-vacío})"
        ((ERRORES++))
    fi

    echo
    echo -e "${NEGRITA}Servicio de apagado${RESET}"
    linea

    if [[ -f "$SERVICIO_MONITOR" ]]; then
        echo -e "${OK} Unidad systemd            : EXISTE"
    else
        echo -e "${ERROR} Unidad systemd            : NO EXISTE ($SERVICIO_MONITOR)"
        ((ERRORES++))
    fi

    if systemctl is-active --quiet "$NOMBRE_SERVICIO" 2>/dev/null; then
        echo -e "${OK} $NOMBRE_SERVICIO : ${VERDE}ACTIVO${RESET}"
    else
        echo -e "${ERROR} $NOMBRE_SERVICIO : ${ROJO}INACTIVO${RESET}"
        ((ERRORES++))
    fi

    if systemctl is-enabled --quiet "$NOMBRE_SERVICIO" 2>/dev/null; then
        echo -e "${OK} Inicio automático        : HABILITADO"
    else
        echo -e "${ERROR} Inicio automático        : DESHABILITADO"
        ((ERRORES++))
    fi

    if [[ -x "$SCRIPT_MONITOR" ]]; then
        echo -e "${OK} Script del monitor       : INSTALADO / EJECUTABLE"
    else
        echo -e "${ERROR} Script del monitor       : NO INSTALADO O SIN PERMISO"
        ((ERRORES++))
    fi

    echo
    echo -e "${NEGRITA}NUT / UPS${RESET}"
    linea

    MODO_NUT=$(detectar_modo_nut)
    echo "Modo NUT                  : $MODO_NUT"

    if [[ "$MODO_NUT" == "netclient" ]]; then
        SERVICIO_NUT_LOCAL="nut-monitor"
        determinar_ups_objetivo
        UPS_OBJETIVO_LOCAL="$UPS_REMOTA"
    else
        SERVICIO_NUT_LOCAL="nut-server"
        UPS_OBJETIVO_LOCAL="$UPS_NAME"
    fi

    if systemctl is-active --quiet "${SERVICIO_NUT_LOCAL}.service" 2>/dev/null; then
        echo -e "${OK} $SERVICIO_NUT_LOCAL            : ${VERDE}ACTIVO${RESET}"
    else
        echo -e "${ERROR} $SERVICIO_NUT_LOCAL            : ${ROJO}INACTIVO${RESET}"
        ((ERRORES++))
    fi

    if [[ -n "$UPS_OBJETIVO_LOCAL" ]] && upsc "$UPS_OBJETIVO_LOCAL" >/dev/null 2>&1; then
        echo -e "${OK} Comunicación UPS         : CORRECTA"
        echo "  UPS                      : $UPS_OBJETIVO_LOCAL"
        ESTADO_UPS=$(upsc "$UPS_OBJETIVO_LOCAL" ups.status 2>/dev/null | head -1)
        BATERIA_UPS=$(upsc "$UPS_OBJETIVO_LOCAL" battery.charge 2>/dev/null | head -1)
        AUTONOMIA_UPS=$(upsc "$UPS_OBJETIVO_LOCAL" battery.runtime 2>/dev/null | head -1)
        echo "  Estado                   : ${ESTADO_UPS:-N/D}"
        echo "  Batería                  : ${BATERIA_UPS:-N/D} %"
        echo "  Autonomía                : ${AUTONOMIA_UPS:-N/D} segundos"
    else
        echo -e "${ERROR} Comunicación UPS         : NO DISPONIBLE (${UPS_OBJETIVO_LOCAL:-sin objetivo})"
        ((ERRORES++))
    fi

    echo
    echo -e "${NEGRITA}Rutina de apagado${RESET}"
    linea

    if declare -F apagar_host >/dev/null 2>&1; then
        echo -e "${OK} Función apagar_host       : DISPONIBLE"
    else
        echo -e "${ERROR} Función apagar_host       : NO DISPONIBLE"
        ((ERRORES++))
    fi

    if command -v shutdown >/dev/null 2>&1; then
        echo -e "${OK} Comando shutdown          : DISPONIBLE"
    else
        echo -e "${ERROR} Comando shutdown          : NO DISPONIBLE"
        ((ERRORES++))
    fi
    ...
    if (( ERRORES == 0 )); then
        if [[ "${MONITOR_APAGADO^^}" == "ON" ]]; then
            echo -e "${VERDE}${NEGRITA}✔ APAGADO AUTOMÁTICO CORRECTAMENTE CONFIGURADO${RESET}"
            echo
            echo "El servidor está preparado para ejecutar el apagado cuando:"
            echo "  - La UPS permanezca en batería después de ${WAIT_POWER_FAIL}s, y"
            echo "  - batería <= ${BATTERY_LIMIT}% o autonomía <= ${RUNTIME_LIMIT}s."
            registrar_log "Verificación apagado automático: ACTIVADO / OK"
        else
            echo -e "${VERDE}${NEGRITA}✔ MONITOREO UPS CORRECTAMENTE CONFIGURADO${RESET}"
            echo
            echo -e "${AMARILLO}${NEGRITA}⚠ APAGADO AUTOMÁTICO BLOQUEADO POR MONITOR_APAGADO=OFF${RESET}"
            echo
            echo "El servicio puede permanecer activo y enviar alertas,"
            echo "pero no ejecutará apagar_host por batería/autonomía."
            registrar_log "Verificación apagado automático: BLOQUEADO (OFF) / monitor OK"
        fi
    else
        echo -e "${ROJO}${NEGRITA}✘ APAGADO AUTOMÁTICO NO ESTÁ LISTO${RESET}"
        echo
        echo "Problemas encontrados: $ERRORES"
        echo "Revise los elementos marcados con ✘ antes de depender del apagado automático."
        registrar_log "Verificación apagado automático: $ERRORES problema(s)"
    fi

    echo
    echo -e "${AMARILLO}Esta verificación NO realiza un apagado de prueba.${RESET}"
    pausa
}
...
actualizar_panel() {
    ...
    if systemctl is-enabled --quiet "$NOMBRE_SERVICIO" 2>/dev/null
    then
        ESTADO_INICIO_MONITOR="${VERDE}HABILITADO${RESET}"
    else
        ESTADO_INICIO_MONITOR="${AMARILLO}DESHABILITADO${RESET}"
    fi

    if [[ "${MONITOR_APAGADO^^}" == "ON" ]]
    then
        ESTADO_APAGADO="${VERDE}ACTIVADO${RESET}"
    else
        ESTADO_APAGADO="${AMARILLO}BLOQUEADO (OFF)${RESET}"
    fi
    ...
    tput cup 35 0
    tput el
    printf "${CYAN}UPS APC Monitor LAN............ :${RESET} %b" "$ESTADO_MONITOR"

    tput cup 36 0
    tput el
    printf "${CYAN}Inicio automático monitor...... :${RESET} %b" "$ESTADO_INICIO_MONITOR"

    tput cup 37 0
    tput el
    printf "${CYAN}Apagado automático............. :${RESET} %b" "$ESTADO_APAGADO"
...
El apagado automático fue cancelado."
            ...
            then
                ...
                then
                    ...
                    # MONITOR_APAGADO controla de forma efectiva si se permite
                    # ejecutar el apagado automático. Las alertas de corte siguen
                    # funcionando aunque esta opción esté en OFF.
                    if [[ "${MONITOR_APAGADO^^}" != "ON" ]]
                    then
                        registrar_log "Límite de apagado alcanzado, pero MONITOR_APAGADO=$MONITOR_APAGADO. Apagado bloqueado."
...
menu_nut_monitor() {
    ...
    do
        header
        titulo "GESTION NUT MONITOR (upsmon)"

        echo
        printf "%-31s " "Estado nut-monitor:"
        nut_monitor_estado_color

        if systemctl is-enabled --quiet nut-monitor.service 2>/dev/null; then
            echo -e "Inicio automático               : ${VERDE}HABILITADO${RESET}"
        else
            echo -e "Inicio automático               : ${AMARILLO}DESHABILITADO${RESET}"
        fi

        if pgrep -x upsmon >/dev/null 2>&1; then
            echo -e "Proceso upsmon                  : ${VERDE}EJECUTÁNDOSE${RESET}"
        else
            echo -e "Proceso upsmon                  : ${ROJO}NO EJECUTÁNDOSE${RESET}"
        fi

        echo
        linea
        echo
        echo -e "${VERDE}1)${RESET} Activar e iniciar NUT Monitor"
        echo -e "${AMARILLO}2)${RESET} Detener y desactivar NUT Monitor"
        echo -e "${CYAN}3)${RESET} Reiniciar NUT Monitor"
        echo -e "${AZUL}4)${RESET} Ver estado detallado"
        echo
        echo -e "${ROJO}0)${RESET} Volver"
        echo
        linea
        ...
        read -rp "Seleccione una opción: " OPCION_NUT_MONITOR

        case "$OPCION_NUT_MONITOR" in
            1)
                systemctl enable --now nut-monitor.service
                if nut_monitor_activo; then
                    mensaje_ok "NUT Monitor activado e iniciado."
                else
                    mensaje_error "NUT Monitor no pudo quedar activo."
                fi
                pausa
            ;;
            2)
                read -rp "¿Detener y desactivar NUT Monitor? [s/N]: " RESP_NUT
                if [[ "$RESP_NUT" =~ ^[sS]$ ]]; then
                    systemctl disable --now nut-monitor.service
                    if ! systemctl is-active --quiet nut-monitor.service 2>/dev/null; then
                        mensaje_ok "NUT Monitor detenido y desactivado."
                    else
                        mensaje_error "NUT Monitor continúa activo."
                    fi
                fi
                pausa
            ;;
            3)
                ...
                if nut_monitor_activo; then
                    mensaje_ok "NUT Monitor reiniciado correctamente."
                else
                    mensaje_error "No fue posible reiniciar NUT Monitor."
                fi
                pausa
            ;;
            4)
                header
                titulo "ESTADO DETALLADO NUT MONITOR"
                echo
                systemctl status nut-monitor.service --no-pager -l
                echo
                linea
                echo
                echo "Proceso upsmon:"
                pgrep -a upsmon || echo "No se encontró ningún proceso upsmon."
                pausa
            ;;
            0) return ;;
            *) mensaje_error "Opción inválida."; sleep 1 ;;
...
############################################################
# ESTADO GENERAL NUT / MONITOR APC LAN / APAGADO
############################################################

panel_sistema_lan() {
    cargar_configuracion
    cargar_email 2>/dev/null
    cargar_telegram 2>/dev/null

    header
    titulo "ESTADO GENERAL UPS - CLIENTE / SERVIDOR LAN"

    MODO_NUT=$(detectar_modo_nut)

    if [[ "$MODO_NUT" == "netclient" ]]; then
        determinar_ups_objetivo
        UPS_OBJETIVO_ESTADO="$UPS_REMOTA"
    else
        UPS_OBJETIVO_ESTADO="$UPS_NAME"
    fi
    ...
    linea
    printf "%-31s %s\n" "Modo NUT:" "$MODO_NUT"
    printf "%-31s %s\n" "UPS objetivo:" "${UPS_OBJETIVO_ESTADO:-N/D}"

    if [[ -n "$UPS_OBJETIVO_ESTADO" ]] && upsc "$UPS_OBJETIVO_ESTADO" >/dev/null 2>&1; then
        echo -e "Comunicación UPS                : ${VERDE}CORRECTA${RESET}"
        ESTADO_G=$(upsc "$UPS_OBJETIVO_ESTADO" ups.status 2>/dev/null | head -1)
        BATERIA_G=$(upsc "$UPS_OBJETIVO_ESTADO" battery.charge 2>/dev/null | head -1)
        AUTONOMIA_G=$(upsc "$UPS_OBJETIVO_ESTADO" battery.runtime 2>/dev/null | head -1)
        CARGA_G=$(upsc "$UPS_OBJETIVO_ESTADO" ups.load 2>/dev/null | head -1)
        MODELO_G=$(upsc "$UPS_OBJETIVO_ESTADO" device.model 2>/dev/null | head -1)

        printf "%-31s %s\n" "Modelo:" "${MODELO_G:-N/D}"
        printf "%-31s %s\n" "Estado UPS:" "${ESTADO_G:-N/D}"
        printf "%-31s %s %%\n" "Batería:" "${BATERIA_G:-N/D}"
        printf "%-31s %s %%\n" "Carga:" "${CARGA_G:-N/D}"
        printf "%-31s %s segundos\n" "Autonomía:" "${AUTONOMIA_G:-N/D}"
    else
        echo -e "Comunicación UPS                : ${ROJO}NO DISPONIBLE${RESET}"
    fi

    mostrar_estado_proteccion_resumido

    echo
    echo -e "${NEGRITA}SERVICIOS NUT${RESET}"
    linea

    if systemctl is-active --quiet nut-server.service 2>/dev/null; then
        echo -e "NUT Server                      : ${VERDE}ACTIVO${RESET}"
    else
        echo -e "NUT Server                      : ${AMARILLO}DETENIDO / NO REQUERIDO${RESET}"
    fi

    printf "%-31s " "NUT Monitor:"
    nut_monitor_estado_color
    ...
    echo
    echo -e "${NEGRITA}DIAGNÓSTICO${RESET}"
    linea

    if ups_apc_monitor_activo; then
        echo -e "${OK} Monitor APC LAN funcionando."
    else
        echo -e "${ERROR} Monitor APC LAN detenido."
    fi

    if [[ "${MONITOR_APAGADO^^}" == "ON" ]]; then
        echo -e "${OK} Apagado automático autorizado."
    else
        echo -e "${OK} Apagado automático bloqueado por configuración."
    fi

    if [[ "$MODO_NUT" == "netclient" ]] && ! nut_monitor_activo; then
        echo -e "${ERROR} En modo netclient NUT Monitor/upsmon debe estar activo."
    elif [[ "$MODO_NUT" == "netclient" ]]; then
        echo -e "${OK} NUT Monitor activo para cliente LAN."
    fi

    echo
    linea
    echo
    echo -e "${CYAN}Gestión rápida:${RESET}"
    echo -e "${VERDE}1)${RESET} Administrar NUT Monitor (upsmon)"
    echo -e "${ROJO}0)${RESET} Volver"
    echo

    read -rp "Seleccione una opción: " OP_ESTADO
    case "$OP_ESTADO" in
        1) menu_nut_monitor ;;
        0|"") return ;;
        *) mensaje_error "Opción inválida."; pausa ;;
...
do
    ...
    echo -e "${AMARILLO}2)${RESET} Estado NUT / Monitores / Protección"
    echo
	...
	echo
    echo -e "${AMARILLO}14)${RESET} Ver estado de alertas + protección"
    ...
    echo -e "${AMARILLO}17)${RESET} Verificar apagado automático UPS"
	
    echo -e "${AMARILLO}18)${RESET} Estado general NUT / Monitor APC LAN / Apagado"
    linea
    ...
    case "$OP" in
        ...
        2)
            header
            titulo "ESTADO NUT / MONITORES / PROTECCIÓN"

            MODO_ESTADO=$(detectar_modo_nut)

            echo
            printf "%-31s %s\n" "Modo NUT:" "$MODO_ESTADO"

            if systemctl is-active --quiet nut-server.service 2>/dev/null
            then
                echo -e "NUT Server                      : ${VERDE}ACTIVO${RESET}"
            else
                echo -e "NUT Server                      : ${AMARILLO}DETENIDO${RESET}"
            fi
        ...
        14)
            header
            titulo "ESTADO DE ALERTAS + PROTECCIÓN"
            estado_alertas
            mostrar_estado_proteccion_resumido
            pausa
        ;;
        ...
        17) verificar_apagado_automatico ;;

        18) panel_sistema_lan ;;
