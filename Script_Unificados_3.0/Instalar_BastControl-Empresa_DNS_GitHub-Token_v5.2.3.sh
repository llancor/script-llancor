...
title(){
  clear
  printf "${C}${B}==================================================\n"
  printf "           BastControl-Empresa DNS v5.1          \n"
...
migrate_legacy_env(){
  ...
  for legacy_env in \
    "/opt/bast-control-dns/bastcontrol/.env" \
    "/opt/bast-control-dns/bastcontrol/.env" \
    "/opt/bast-control-dns/BastControl/.env"; do
...
download_project(){
  ...
  title
  printf "${B}Descargando BastControl-Empresa DNS v5.1 desde GitHub...${N}\n\n"
  ...
  temp_repo="$(mktemp -d /tmp/bastcontrol-empresa-dns.XXXXXX)" || return 1
...
"
  ...
  # Copia únicamente BastControl-Empresa_DNS_v5.1 en la ruta final.
...
install_app(){
  ...
  printf "\n${B}Construyendo y arrancando BastControl-Empresa DNS v5.1...${N}\n"
...
verificar_pv() {
    ...
    if command -v pv >/dev/null 2>&1; then
        ...
        printf "${G}✓ pv instalado${N}\n"
        return

    fi

    echo
    printf "${Y}Instalando pv...${N}\n"
    echo
...
listar_apps() {
    ...
    echo
    printf "${C}${B}APLICACIONES ENCONTRADAS:${N}\n"
    echo
...
update_app(){
  ...
  if dc up -d --build --remove-orphans; then
    printf "${G}BastControl-Empresa DNS v5.1 fue actualizado correctamente. La base de datos y configuración se conservaron.${N}\n"
    dc ps
    show_url
  else
...
while true; do
  ...
  printf "  ${Y}2)${C} Instalar o migrar a BastControl-Empresa GitHub-Token${N}\n\n"
  ...
  printf "  ${Y}10)${Y} Actualizar BastControl-Empresa DNSG GitHub-Token${N}\n"
  printf "  ${Y}11)${G} Crear respaldo completo (proyecto, DB y volúmenes)${N}\n"
