...
# Papelera / Versiones / Uploads - borrado directo
# Conserva las carpetas base y elimina solo su contenido
# =========================================================
menu_limpieza_nextcloud(){
    ...
    _nc_limpiar_categoria(){
        local nombre="$1"
        local dir encontrados=0

        while IFS= read -r -d '' dir; do
            encontrados=1
            # Conserva files_trashbin / files_versions / uploads.
            # Elimina únicamente todo lo que contienen.
    ...
    while true; do
        ...
        DATA_DIR_LIMPIEZA=$(sudo -u "$USER_WEB" php "$NEXTCLOUD_DIR/occ" \
            config:system:get datadirectory 2>/dev/null)
        [[ -z "$DATA_DIR_LIMPIEZA" ]] && DATA_DIR_LIMPIEZA="${NEXTCLOUD_DATA:-/var/www/nextcloud-data}"
        ...
        case "$op_limpieza" in
            1)
                ...
                _nc_limpiar_categoria files_trashbin
            ...
            2)
                ...
                confirmar "¿Eliminar TODAS las versiones de todos los usuarios?" || continue
                ANTES=$(_nc_size_categoria files_versions)
                _nc_limpiar_categoria files_versions
            ...
            ;;
            5)
                [[ "$SIZE_TOTAL" -eq 0 ]] && { ok "No hay espacio que limpiar."; pausa; continue; }
                echo
                echo -e "${RED}⚠ Se vaciará papelera, versiones y uploads temporales.${NC}"
                echo -e "${YELLOW}No debe haber cargas de archivos activas.${NC}"
                echo -e "${CYAN}Espacio detectado:${NC} $(_nc_human_bytes "$SIZE_TOTAL")"
                echo
                confirmar "¿CONFIRMAR LIMPIEZA TOTAL?" || continue

                ANTES_TOTAL=$SIZE_TOTAL
                _nc_limpiar_categoria files_trashbin
                _nc_limpiar_categoria files_versions
                _nc_limpiar_categoria uploads
                ...
                echo -e "${GREEN}Espacio restante:${NC} $(_nc_human_bytes "$DESPUES_TOTAL")"
                echo -e "${GREEN}════════════════════════════════════════════${NC}"
                pausa
            ;;
            0) return ;;
...
echo
echo -e "${CYAN}====================================================${RESET}"
echo -e "${CYAN}       NEXTCLOUD PRO v8.3 PRO VIP GitHub V1.4.9   ${RESET}"
echo -e "${CYAN}====================================================${RESET}"
echo
