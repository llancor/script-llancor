...
# Usuario NUT autorizado para ejecutar comandos instantáneos (upscmd)
NUT_CMD_USER="monitor"
NUT_CMD_PASSWORD="monitor"
NUT_USERS_FILE="/etc/nut/upsd.users"
...
# VARIABLES
############################################################
SCRIPT="/root/Gestion_UPS_APC-SRV3KI_v5.1.sh"
...
nut_driver_activo() {
    systemctl is-active --quiet nut-driver@apc.service 2>/dev/null ||
    pgrep -f 'nutdrv_qx.*apc|nutdrv_qx' >/dev/null 2>&1
}

nut_server_activo() {
    systemctl is-active --quiet nut-server.service 2>/dev/null
}

mostrar_estado_conexion_nut() {
    local TIPO PUERTO DRIVER
    TIPO="$(tipo_conexion_ups)"
    PUERTO="$(obtener_puerto_ups)"
    DRIVER="$(obtener_driver_ups)"

    printf "%-31s %s\n" "Conexión física UPS:" "${TIPO:-NO DETECTADA}"
    printf "%-31s %s\n" "Puerto UPS:" "${PUERTO:-NO CONFIGURADO}"
    printf "%-31s %s\n" "Driver configurado:" "${DRIVER:-NO CONFIGURADO}"

    printf "%-31s " "NUT Driver:"
    if nut_driver_activo; then
        echo -e "${VERDE}ACTIVO${RESET}"
    else
        echo -e "${ROJO}DETENIDO${RESET}"
    fi

    printf "%-31s " "NUT Server:"
    if nut_server_activo; then
        echo -e "${VERDE}ACTIVO${RESET}"
    else
        echo -e "${ROJO}DETENIDO${RESET}"
    fi
...
nut_monitor_activo() {
    # Primero comprobar la unidad systemd oficial.
    if systemctl is-active --quiet nut-monitor.service 2>/dev/null; then
        return 0
    fi

    # Compatibilidad: en algunas instalaciones upsmon puede estar ejecutándose
    # aunque systemd no informe la unidad como activa.
...
estado_apagado_color() {
    cargar_configuracion
    if [[ "$MONITOR_APAGADO" == "ON" ]]; then
        echo -e "${VERDE}ACTIVADO${RESET}"
    else
        echo -e "${AMARILLO}BLOQUEADO (OFF)${RESET}"
    fi
}
...
actualizar_panel() {
    ...
    ####################################################
    # ESTADO DE MONITORES (separados para evitar confusión)
    ####################################################

    if nut_monitor_activo; then
        if systemctl is-active --quiet nut-monitor.service 2>/dev/null; then
            ESTADO_NUT_MONITOR="${VERDE}ACTIVO${RESET}"
        else
            ESTADO_NUT_MONITOR="${AMARILLO}ACTIVO (upsmon)${RESET}"
        fi
    else
        ESTADO_NUT_MONITOR="${ROJO}DETENIDO${RESET}"
    fi

    if ups_apc_monitor_activo; then
        ESTADO_SERVICIO_APC="${VERDE}ACTIVO${RESET}"
    else
        ESTADO_SERVICIO_APC="${ROJO}DETENIDO${RESET}"
    fi

    if ups_apc_monitor_habilitado; then
        ESTADO_INICIO_APC="${VERDE}HABILITADO${RESET}"
    else
        ESTADO_INICIO_APC="${AMARILLO}DESHABILITADO${RESET}"
    fi
    ...
    ########################################################
    # ESTADO DE MONITORES Y APAGADO
    ########################################################

    tput cup 34 0
    tput el
    echo -e "${CYAN}Estado de protección / monitoreo - UPS: $(tipo_conexion_ups)${RESET}"

    tput cup 35 0
    tput el
    printf "${CYAN}NUT Monitor (upsmon)......... :${RESET} %b" "$ESTADO_NUT_MONITOR"

    tput cup 36 0
    tput el
    printf "${CYAN}UPS APC Monitor.............. :${RESET} %b" "$ESTADO_SERVICIO_APC"

    tput cup 37 0
    tput el
    printf "${CYAN}Inicio automático monitor.... :${RESET} %b" "$ESTADO_INICIO_APC"
...
echo
    ...
    linea
    echo
    echo -e "${CYAN}Conexión y servicios NUT${RESET}"
    mostrar_estado_conexion_nut
    echo
    linea
...
############################################################
# CONFIGURAR / REPARAR USUARIO NUT PARA COMANDOS UPS
############################################################

configurar_usuario_comandos_nut() {

    local ARCHIVO="$NUT_USERS_FILE"
    local TMP
    local BACKUP
    local BLOQUE
    ...
    # Comprobar si el bloque [monitor] ya tiene exactamente los permisos necesarios.
    if [[ -f "$ARCHIVO" ]]; then
        BLOQUE=$(awk '
            /^\[monitor\][[:space:]]*$/ {captura=1; next}
            /^\[[^]]+\][[:space:]]*$/ {if (captura) exit}
            captura {print}
        ' "$ARCHIVO")

        if grep -Eq '^[[:space:]]*password[[:space:]]*=[[:space:]]*monitor[[:space:]]*$' <<< "$BLOQUE" &&
           grep -Eq '^[[:space:]]*upsmon[[:space:]]+primary[[:space:]]*$' <<< "$BLOQUE" &&
           grep -Eq '^[[:space:]]*actions[[:space:]]*=[[:space:]]*SET[[:space:]]*$' <<< "$BLOQUE" &&
           grep -Eq '^[[:space:]]*instcmds[[:space:]]*=[[:space:]]*ALL[[:space:]]*$' <<< "$BLOQUE"; then
            return 0
        fi
    fi

    echo
    echo -e "${AMARILLO}Configurando permisos NUT para controlar la UPS...${RESET}"

    # Guardar respaldo antes de modificar.
    if [[ -f "$ARCHIVO" ]]; then
        mkdir -p "$BACKUP_DIR"
        BACKUP="$BACKUP_DIR/upsd.users.$(date '+%Y%m%d-%H%M%S').bak"
        cp -a "$ARCHIVO" "$BACKUP" || {
            mensaje_error "No fue posible respaldar $ARCHIVO."
            return 1
        }
    ...
    if [[ -f "$ARCHIVO" ]]; then
        # Sustituye únicamente el bloque [monitor] y conserva los demás usuarios.
        awk '
            BEGIN { dentro=0; escrito=0 }
            /^\[monitor\][[:space:]]*$/ {
                if (!escrito) {
                    print "[monitor]"
                    print "password = monitor"
                    print "upsmon primary"
                    print "actions = SET"
                    print "instcmds = ALL"
                    escrito=1
                }
...
EOF
    ...
    chown root:nut "$ARCHIVO" 2>/dev/null || chown root:root "$ARCHIVO" 2>/dev/null
    chmod 640 "$ARCHIVO"

    if ! systemctl restart nut-server.service >/dev/null 2>&1; then
        mensaje_error "No fue posible reiniciar nut-server después de configurar permisos."
        return 1
    fi
    ...
    if ! systemctl is-active --quiet nut-server.service; then
        mensaje_error "nut-server no quedó activo."
        return 1
    fi

    registrar_log "Usuario NUT monitor configurado con actions=SET e instcmds=ALL."
    echo -e "${VERDE}✔ Permisos NUT configurados correctamente.${RESET}"

    return 0
}

############################################################
# CONTROL DIRECTO UPS / BEEPER
############################################################
...
estado_beeper() {
    ...
    ESTADO_BEEPER=$(upsc "$UPS_NAME" ups.beeper.status 2>/dev/null | tr -d '\r')

    case "$ESTADO_BEEPER" in
        enabled)
            echo -e "${VERDE}ACTIVADO${RESET}"
        ;;
        disabled)
            echo -e "${ROJO}DESACTIVADO${RESET}"
        ;;
        muted)
            echo -e "${AMARILLO}SILENCIADO${RESET}"
        ;;
        *)
            echo -e "${AMARILLO}${ESTADO_BEEPER:-NO DISPONIBLE}${RESET}"
        ;;
    esac
}
...
comando_beeper_disponible() {

    local COMANDO="$1"

    upscmd -l "$UPS_NAME" 2>/dev/null | grep -q "^${COMANDO}[[:space:]]"
}
...
controlar_beeper() {
    ...
    if ! command -v upscmd >/dev/null 2>&1; then
        mensaje_error "El comando upscmd no está disponible."
        pausa
        return 1
    fi

    if ! comando_beeper_disponible "$COMANDO"; then
        mensaje_error "La UPS no informa soporte para '$COMANDO'."
        pausa
        return 1
    fi

    echo
    echo -e "${CYAN}${DESCRIPCION}${RESET}"
    echo

    # Asegurar autenticación y permisos para comandos antes de llamar a upscmd.
    if ! configurar_usuario_comandos_nut; then
        mensaje_error "No fue posible preparar el usuario de control NUT."
        pausa
        return 1
    fi

    SALIDA=$(upscmd -u "$NUT_CMD_USER" -p "$NUT_CMD_PASSWORD" "$UPS_NAME" "$COMANDO" 2>&1)
    RESULTADO=$?

    if [[ $RESULTADO -eq 0 ]]; then
        sleep 1
        mensaje_ok "$DESCRIPCION ejecutado correctamente."
        echo
        printf "%-28s " "Estado actual del beeper:"
        estado_beeper
    else
        mensaje_error "No fue posible ejecutar '$COMANDO'."
        echo
        echo "$SALIDA"
        echo
        echo -e "${AMARILLO}Revise $NUT_USERS_FILE y el estado de nut-server.${RESET}"
    fi
...
menu_control_ups() {
    ...
    do
        ...
        titulo "CONTROL DIRECTO DE LA UPS"
        ...
        echo
        echo -e "${CYAN}Modelo:${RESET}          ${MODELO:-No disponible}"
        echo -e "${CYAN}Estado UPS:${RESET}      ${ESTADO:-No disponible}"
        echo -e "${CYAN}Batería:${RESET}         ${BATERIA:-0}%"
        echo -e "${CYAN}Carga:${RESET}           ${CARGA:-0}%"
        printf "${CYAN}Pitido / Beeper:${RESET} "
        estado_beeper

        echo
        linea
        echo
        echo -e "${VERDE}1)${RESET} Ver estado rápido de la UPS"
        echo -e "${VERDE}2)${RESET} Desactivar pitido / beeper"
        echo -e "${VERDE}3)${RESET} Activar pitido / beeper"
        echo -e "${VERDE}4)${RESET} Alternar estado del pitido"
        echo -e "${VERDE}5)${RESET} Ver comandos soportados por la UPS"
        echo
        echo -e "${ROJO}0)${RESET} Volver"
        echo
        linea

        read -rp "Seleccione una opción: " OPCION_CONTROL

        case "$OPCION_CONTROL" in
            1)
                header
                mostrar_estado
                pausa
            ...
            2)
                controlar_beeper "beeper.disable" "Desactivar pitido de la UPS"
            ;;

            3)
                controlar_beeper "beeper.enable" "Activar pitido de la UPS"
            ;;

            4)
                controlar_beeper "beeper.toggle" "Alternar pitido de la UPS"
            ;;

            5)
                header
                titulo "COMANDOS SOPORTADOS POR LA UPS"
                echo
                upscmd -l "$UPS_NAME" 2>&1
                echo
                echo -e "${ROJO}ADVERTENCIA:${RESET} No ejecute load.off, shutdown.*, driver.killpower o bypass.* sin conocer su efecto."
                pausa
            ;;
...
configurar_shutdown() {
    ...
    if [[ -n "$TMP" ]]; then
        TMP="${TMP^^}"
        if [[ "$TMP" == "ON" || "$TMP" == "OFF" ]]; then
            MONITOR_APAGADO="$TMP"
        else
            mensaje_warning "Valor inválido. Se conservará MONITOR_APAGADO=$MONITOR_APAGADO"
        fi
    fi
...
Servidor: $(hostname)"
                    ...
                    if [[ "$MONITOR_APAGADO" == "ON" ]]; then
                        registrar_log "MONITOR_APAGADO=ON: se autoriza el apagado automático."
                        apagar_host
                    else
                        registrar_log "Límite de UPS alcanzado, pero MONITOR_APAGADO=OFF. Apagado bloqueado."
                        enviar_correo \
                        "UPS APC - Límite alcanzado (apagado desactivado)" \
"La UPS alcanzó el límite configurado, pero el apagado automático está DESACTIVADO.
...
MONITOR_APAGADO=OFF. El servidor NO será apagado."

                        enviar_telegram \
"⚠️ UPS APC

Límite de apagado alcanzado, pero el apagado automático está DESACTIVADO.
...
menu_nut_monitor() {
    ...
    do
        ...
        case "$OPCION_NUT_MONITOR" in
            ...
            2)
                echo
                if confirmar "¿Detener y desactivar NUT Monitor?"; then
...
panel_sistema() {
    ...
    ESTADO_UPS=$(obtener_valor ups.status)
    MODELO_UPS=$(obtener_valor device.model)
    BATERIA_UPS=$(obtener_valor battery.charge)
    CARGA_UPS=$(obtener_valor ups.load)
    AUTONOMIA_UPS=$(obtener_valor battery.runtime)
    TEMP_UPS=$(obtener_valor ups.temperature)

    printf "%-28s %s\n" "Modelo:" "${MODELO_UPS:-N/D}"
    printf "%-28s %s\n" "Estado:" "${ESTADO_UPS:-N/D}"
    printf "%-28s %s %%\n" "Batería:" "${BATERIA_UPS:-N/D}"
    printf "%-28s %s %%\n" "Carga:" "${CARGA_UPS:-N/D}"
    printf "%-28s %s segundos\n" "Autonomía:" "${AUTONOMIA_UPS:-N/D}"
    printf "%-28s %s °C\n" "Temperatura:" "${TEMP_UPS:-N/D}"
    ...
    if ups_apc_monitor_activo; then
        echo -e "UPS APC Monitor             : ${VERDE}ACTIVO${RESET}"
    else
        echo -e "UPS APC Monitor             : ${ROJO}DETENIDO${RESET}"
    fi

    if ups_apc_monitor_habilitado; then
        echo -e "Inicio UPS APC Monitor      : ${VERDE}HABILITADO${RESET}"
    else
        echo -e "Inicio UPS APC Monitor      : ${AMARILLO}DESHABILITADO${RESET}"
    fi

    echo
    echo "APAGADO AUTOMÁTICO"
    echo "────────────────────────────────"
    ...
    if upsc "$UPS_NAME" >/dev/null 2>&1; then
        echo -e "${VERDE}✔${RESET} Comunicación con la UPS correcta."
    else
        echo -e "${ROJO}✘${RESET} No hay comunicación con la UPS."
    fi

    if ups_apc_monitor_activo; then
        echo -e "${VERDE}✔${RESET} El monitor APC personalizado está funcionando."
    else
        echo -e "${ROJO}✘${RESET} El monitor APC personalizado está detenido."
    fi

    if [[ "$MONITOR_APAGADO" == "ON" ]]; then
        echo -e "${VERDE}✔${RESET} El apagado automático está autorizado."
    else
        echo -e "${VERDE}✔${RESET} El apagado automático está bloqueado por configuración."
    fi

    if ! nut_monitor_activo; then
        echo
        echo -e "${AMARILLO}NOTA:${RESET} nut-monitor está detenido, pero es distinto de"
        echo "ups-apc-monitor.service. El monitor APC personalizado puede"
        echo "seguir funcionando y enviando alertas aunque nut-monitor esté detenido."
    fi
    ...
    echo -e "${ROJO}0)${RESET} Volver"
    echo
    read -rp "Seleccione una opción: " OPCION_ESTADO

    case "$OPCION_ESTADO" in
        1) menu_nut_monitor ;;
        0|"") return ;;
        *) mensaje_warning "Opción inválida." ; pausa ;;
    esac
}
...
menu() {
    ...
    do
        ...
        echo -e "${VERDE}1)${RESET} Control UPS / Activar-Desactivar pitido"
        echo -e "${VERDE}2)${RESET} Monitor en tiempo real + estado de protección"
		...
		echo
        echo -e "${VERDE}14)${AMARILLO} Monitor manual de corte/apagado + estado ${VERDE}* Mensajes Automáticos"
		echo
		...
		echo
		echo -e "${VERDE}18)${VERDE} Estado general NUT / Monitor APC / Apagado"
		echo
