...
SHINOBI_IMAGE="registry.gitlab.com/shinobi-systems/shinobi:latest"
...
EOF
    ...
    if docker compose up -d; then
        ...
        if esperar_shinobi; then
            ...
            cat > "$CRED_FILE" <<CREDS
Usuario: $DEFAULT_SUPER_USER
Contraseña: $DEFAULT_SUPER_PASS
URL Super: http://${IP}:${HTTP_PORT}/super
CREDS
            ...
            echo "Usuario:"
            echo "$DEFAULT_SUPER_USER"


            echo

            echo "Clave:"
            echo "$DEFAULT_SUPER_PASS"
...
for f in /home/Shinobi/super.json /config/super.json; do
  [ -f "$f" ] && { echo "$f"; exit 0; }
done
find /home/Shinobi /config -maxdepth 3 -name super.json -type f 2>/dev/null | head -n1
' 2>/dev/null
}

ver_credenciales_super() {
    header
    titulo "CREDENCIALES SHINOBI"

    local IP SUPER_JSON
    IP=$(hostname -I | awk '{print $1}')

    echo "Panel principal : http://${IP}:${HTTP_PORT}"
    echo "Panel Super     : http://${IP}:${HTTP_PORT}/super"
    echo "Puerto Web      : $HTTP_PORT"
    echo "Directorio      : $BASE_DIR"
    echo

    if [ -f "$CRED_FILE" ]; then
        echo -e "${CYAN}Credenciales guardadas:${NC}"
        cat "$CRED_FILE"
    else
        echo "Usuario inicial : $DEFAULT_SUPER_USER"
        echo "Clave inicial   : $DEFAULT_SUPER_PASS"
    fi

    echo
    if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        SUPER_JSON=$(obtener_super_json)
        if [ -n "$SUPER_JSON" ]; then
            echo -e "${CYAN}Superusuarios registrados:${NC}"
            docker exec -e SJ="$SUPER_JSON" "$CONTAINER_NAME" node -e '
const fs=require("fs");
let a=JSON.parse(fs.readFileSync(process.env.SJ,"utf8"));
if(!Array.isArray(a))a=[a];
a.forEach((u,i)=>console.log(`  ${i+1}) ${u.mail||u.email||"(sin correo)"}`));
' 2>/dev/null || true
        fi
    fi
...
cambiar_superusuario() {
    header
    titulo "CAMBIAR SUPERUSUARIO"

    if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        echo -e "${RED}Shinobi no está en ejecución.${NC}"
        pause
        return
    fi

    local SUPER_JSON NUEVO_CORREO PASS PASS2 HASH
    SUPER_JSON=$(obtener_super_json)
    if [ -z "$SUPER_JSON" ]; then
        echo -e "${RED}No se encontró super.json.${NC}"
        pause
        return
    fi
    ...
    docker exec -e SJ="$SUPER_JSON" "$CONTAINER_NAME" node -e '
const fs=require("fs");let a=JSON.parse(fs.readFileSync(process.env.SJ,"utf8"));if(!Array.isArray(a))a=[a];a.forEach((u,i)=>console.log(`${i+1}) ${u.mail||u.email||""}`));
'
    echo
    read -rp "Seleccione número de superusuario [1]: " IDX
    IDX=${IDX:-1}
    read -rp "Nuevo correo (ENTER = mantener actual): " NUEVO_CORREO
    read -rsp "Nueva contraseña: " PASS; echo
    read -rsp "Repita nueva contraseña: " PASS2; echo

    if [ -z "$PASS" ] || [ "$PASS" != "$PASS2" ]; then
        echo -e "${RED}Las contraseñas no coinciden o están vacías.${NC}"
        pause
        return
    fi

    HASH=$(printf '%s' "$PASS" | md5sum | awk '{print $1}')

    if docker exec -e SJ="$SUPER_JSON" -e IDX="$IDX" -e MAIL="$NUEVO_CORREO" -e HASH="$HASH" "$CONTAINER_NAME" node -e '
const fs=require("fs");const p=process.env.SJ;let a=JSON.parse(fs.readFileSync(p,"utf8"));if(!Array.isArray(a))a=[a];const i=(parseInt(process.env.IDX||"1",10)-1);if(i<0||i>=a.length)process.exit(2);if(process.env.MAIL)a[i].mail=process.env.MAIL;a[i].pass=process.env.HASH;fs.writeFileSync(p,JSON.stringify(a,null,3));
'; then
        local EMAIL_GUARDADO
        EMAIL_GUARDADO=$(docker exec -e SJ="$SUPER_JSON" -e IDX="$IDX" "$CONTAINER_NAME" node -e '
const fs=require("fs");let a=JSON.parse(fs.readFileSync(process.env.SJ,"utf8"));if(!Array.isArray(a))a=[a];const i=(parseInt(process.env.IDX||"1",10)-1);console.log(a[i].mail||a[i].email||"");
')
        cat > "$CRED_FILE" <<CREDS
Usuario: $EMAIL_GUARDADO
Contraseña: $PASS
URL Super: http://$(hostname -I | awk '{print $1}'):${HTTP_PORT}/super
CREDS
        chmod 600 "$CRED_FILE" 2>/dev/null || true
        docker restart "$CONTAINER_NAME" >/dev/null
        echo
        echo -e "${GREEN}✓ Superusuario actualizado correctamente.${NC}"
        echo "Usuario: $EMAIL_GUARDADO"
        echo "Clave  : $PASS"
    else
...
crear_superusuario() {
    ...
    local SUPER_JSON MAIL PASS PASS2 HASH
    SUPER_JSON=$(obtener_super_json)
    [ -n "$SUPER_JSON" ] || { echo -e "${RED}No se encontró super.json.${NC}"; pause; return; }

    read -rp "Correo del nuevo superusuario: " MAIL
    read -rsp "Contraseña: " PASS; echo
    read -rsp "Repita contraseña: " PASS2; echo
    [ -n "$MAIL" ] && [ -n "$PASS" ] && [ "$PASS" = "$PASS2" ] || { echo -e "${RED}Datos inválidos.${NC}"; pause; return; }

    HASH=$(printf '%s' "$PASS" | md5sum | awk '{print $1}')
    if docker exec -e SJ="$SUPER_JSON" -e MAIL="$MAIL" -e HASH="$HASH" "$CONTAINER_NAME" node -e '
const fs=require("fs");const p=process.env.SJ;let a=JSON.parse(fs.readFileSync(p,"utf8"));if(!Array.isArray(a))a=[a];if(a.some(u=>(u.mail||u.email)===process.env.MAIL))process.exit(3);a.push({mail:process.env.MAIL,pass:process.env.HASH});fs.writeFileSync(p,JSON.stringify(a,null,3));
'; then
        docker restart "$CONTAINER_NAME" >/dev/null
...
menu_usuarios() {
    while true; do
        ...
        echo -e " ${YELLOW}1)${NC} Ver credenciales y URLs"
        echo -e " ${YELLOW}2)${NC} Cambiar correo / contraseña de Superusuario"
        echo -e " ${YELLOW}3)${NC} Crear Superusuario adicional"
        echo -e " ${YELLOW}4)${NC} Crear usuario Admin de cámaras (abrir /super)"
        echo -e " ${YELLOW}0)${NC} Volver"
        echo
        read -rp "Seleccione una opción: " OPCION_USUARIO
        case "$OPCION_USUARIO" in
            ...
            4)
                header
                titulo "CREAR USUARIO ADMIN DE CÁMARAS"
                echo "Ingrese en: http://$(hostname -I | awk '{print $1}'):${HTTP_PORT}/super"
                echo "Desde el panel Superuser cree la cuenta Admin que administrará cámaras."
                pause
                ;;
            0) return ;;
...
menu_principal() {
    ...
    while true; do
        ...
        echo -e " ${YELLOW}6)${NC} Gestión de usuarios"
        echo -e " ${YELLOW}7)${NC} Información"
        echo -e " ${YELLOW}8)${NC} Desinstalar Shinobi"
        echo -e " ${YELLOW}9)${NC} Cambiar Idioma Shinobi"
        echo
        ...
        case "$opcion" in
            ...
            2) instalar_shinobi ;;
            3) estado_shinobi ;;
            4) administrar_servicios ;;
            5) menu_ajustes ;;
            6) menu_usuarios ;;
            7) informacion ;;
            8) desinstalar_shinobi ;;
            9) cambiar_idioma_shinobi ;;
            0) clear; exit 0 ;;
